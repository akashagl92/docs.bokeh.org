(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error() {
          console.error("failed to load " + url);
        }
    
        for (var i = 0; i < css_urls.length; i++) {
          var url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error;
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        for (var i = 0; i < js_urls.length; i++) {
          var url = js_urls[i];
          var element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error;
          element.async = false;
          element.src = url;
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };var element = document.getElementById("46c20642-7b06-4f83-90be-db5a8c1ad164");
      if (element == null) {
        console.error("Bokeh: ERROR: autoload.js configured with elementid '46c20642-7b06-4f83-90be-db5a8c1ad164' but no matching script tag was found. ")
        return false;
      }
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      var js_urls = ["https://cdn.pydata.org/bokeh/release/bokeh-1.3.2.min.js", "https://cdn.pydata.org/bokeh/release/bokeh-widgets-1.3.2.min.js", "https://cdn.pydata.org/bokeh/release/bokeh-tables-1.3.2.min.js", "https://cdn.pydata.org/bokeh/release/bokeh-gl-1.3.2.min.js", "https://cdn.pydata.org/bokeh/release/bokeh-api-1.3.2.min.js"];
      var css_urls = [];
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"3c077457-f4b6-470b-99e2-393348247ab0":{"roots":{"references":[{"attributes":{"formatter":{"id":"31697","type":"BasicTickFormatter"},"ticker":{"id":"31627","type":"BasicTicker"}},"id":"31626","type":"LinearAxis"},{"attributes":{"overlay":{"id":"31714","type":"BoxAnnotation"}},"id":"31678","type":"BoxZoomTool"},{"attributes":{},"id":"31664","type":"LinearScale"},{"attributes":{},"id":"31624","type":"LinearScale"},{"attributes":{"fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"xs_units":"screen","ys_units":"screen"},"id":"31716","type":"PolyAnnotation"},{"attributes":{"dimension":1,"ticker":{"id":"31632","type":"BasicTicker"}},"id":"31635","type":"Grid"},{"attributes":{},"id":"31679","type":"ResetTool"},{"attributes":{"tools":[{"id":"31636","type":"PanTool"},{"id":"31637","type":"WheelZoomTool"},{"id":"31638","type":"BoxZoomTool"},{"id":"31639","type":"ResetTool"},{"id":"31640","type":"SaveTool"},{"id":"31641","type":"BoxSelectTool"},{"id":"31642","type":"LassoSelectTool"},{"id":"31676","type":"PanTool"},{"id":"31677","type":"WheelZoomTool"},{"id":"31678","type":"BoxZoomTool"},{"id":"31679","type":"ResetTool"},{"id":"31680","type":"SaveTool"},{"id":"31681","type":"BoxSelectTool"},{"id":"31682","type":"LassoSelectTool"}]},"id":"31719","type":"ProxyToolbar"},{"attributes":{"callback":null},"id":"31660","type":"DataRange1d"},{"attributes":{},"id":"31680","type":"SaveTool"},{"attributes":{"toolbar":{"id":"31719","type":"ProxyToolbar"},"toolbar_location":"above"},"id":"31720","type":"ToolbarBox"},{"attributes":{"ticker":{"id":"31667","type":"BasicTicker"}},"id":"31670","type":"Grid"},{"attributes":{"source":{"id":"31615","type":"ColumnDataSource"}},"id":"31655","type":"CDSView"},{"attributes":{"callback":null,"overlay":{"id":"31715","type":"BoxAnnotation"}},"id":"31681","type":"BoxSelectTool"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#1f77b4"},"line_alpha":{"value":0.1},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y0"}},"id":"31653","type":"Circle"},{"attributes":{"children":[{"id":"31720","type":"ToolbarBox"},{"id":"31718","type":"GridBox"}]},"id":"31721","type":"Column"},{"attributes":{"callback":null,"overlay":{"id":"31705","type":"PolyAnnotation"}},"id":"31642","type":"LassoSelectTool"},{"attributes":{"callback":null,"overlay":{"id":"31716","type":"PolyAnnotation"}},"id":"31682","type":"LassoSelectTool"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"31714","type":"BoxAnnotation"},{"attributes":{"formatter":{"id":"31708","type":"BasicTickFormatter"},"ticker":{"id":"31667","type":"BasicTicker"}},"id":"31666","type":"LinearAxis"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"31676","type":"PanTool"},{"id":"31677","type":"WheelZoomTool"},{"id":"31678","type":"BoxZoomTool"},{"id":"31679","type":"ResetTool"},{"id":"31680","type":"SaveTool"},{"id":"31681","type":"BoxSelectTool"},{"id":"31682","type":"LassoSelectTool"}]},"id":"31683","type":"Toolbar"},{"attributes":{"callback":null},"id":"31658","type":"DataRange1d"},{"attributes":{"data_source":{"id":"31615","type":"ColumnDataSource"},"glyph":{"id":"31692","type":"Circle"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"31693","type":"Circle"},"selection_glyph":null,"view":{"id":"31695","type":"CDSView"}},"id":"31694","type":"GlyphRenderer"},{"attributes":{},"id":"31672","type":"BasicTicker"},{"attributes":{"callback":null,"data":{"x":{"__ndarray__":"","dtype":"float64","shape":[300]},"y0":{"__ndarray__":"","dtype":"float64","shape":[300]},"y1":{"__ndarray__":"","dtype":"float64","shape":[300]}},"selected":{"id":"31702","type":"Selection"},"selection_policy":{"id":"31701","type":"UnionRenderers"}},"id":"31615","type":"ColumnDataSource"},{"attributes":{},"id":"31636","type":"PanTool"},{"attributes":{},"id":"31677","type":"WheelZoomTool"},{"attributes":{"below":[{"id":"31666","type":"LinearAxis"}],"center":[{"id":"31670","type":"Grid"},{"id":"31675","type":"Grid"}],"left":[{"id":"31671","type":"LinearAxis"}],"plot_height":350,"plot_width":350,"renderers":[{"id":"31694","type":"GlyphRenderer"}],"title":null,"toolbar":{"id":"31683","type":"Toolbar"},"toolbar_location":null,"x_range":{"id":"31658","type":"DataRange1d"},"x_scale":{"id":"31662","type":"LinearScale"},"y_range":{"id":"31660","type":"DataRange1d"},"y_scale":{"id":"31664","type":"LinearScale"}},"id":"31656","subtype":"Figure","type":"Plot"},{"attributes":{"source":{"id":"31615","type":"ColumnDataSource"}},"id":"31695","type":"CDSView"},{"attributes":{},"id":"31667","type":"BasicTicker"},{"attributes":{"fill_color":{"value":"#1f77b4"},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y1"}},"id":"31692","type":"Circle"},{"attributes":{"children":[[{"id":"31616","subtype":"Figure","type":"Plot"},0,0],[{"id":"31656","subtype":"Figure","type":"Plot"},0,1]]},"id":"31718","type":"GridBox"},{"attributes":{"formatter":{"id":"31699","type":"BasicTickFormatter"},"ticker":{"id":"31632","type":"BasicTicker"}},"id":"31631","type":"LinearAxis"},{"attributes":{},"id":"31632","type":"BasicTicker"},{"attributes":{},"id":"31697","type":"BasicTickFormatter"},{"attributes":{"dimension":1,"ticker":{"id":"31672","type":"BasicTicker"}},"id":"31675","type":"Grid"},{"attributes":{},"id":"31637","type":"WheelZoomTool"},{"attributes":{"below":[{"id":"31626","type":"LinearAxis"}],"center":[{"id":"31630","type":"Grid"},{"id":"31635","type":"Grid"}],"left":[{"id":"31631","type":"LinearAxis"}],"plot_height":350,"plot_width":350,"renderers":[{"id":"31654","type":"GlyphRenderer"}],"title":null,"toolbar":{"id":"31643","type":"Toolbar"},"toolbar_location":null,"x_range":{"id":"31618","type":"DataRange1d"},"x_scale":{"id":"31622","type":"LinearScale"},"y_range":{"id":"31620","type":"DataRange1d"},"y_scale":{"id":"31624","type":"LinearScale"}},"id":"31616","subtype":"Figure","type":"Plot"},{"attributes":{"fill_color":{"value":"#1f77b4"},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y0"}},"id":"31652","type":"Circle"},{"attributes":{},"id":"31699","type":"BasicTickFormatter"},{"attributes":{},"id":"31710","type":"BasicTickFormatter"},{"attributes":{},"id":"31701","type":"UnionRenderers"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"31636","type":"PanTool"},{"id":"31637","type":"WheelZoomTool"},{"id":"31638","type":"BoxZoomTool"},{"id":"31639","type":"ResetTool"},{"id":"31640","type":"SaveTool"},{"id":"31641","type":"BoxSelectTool"},{"id":"31642","type":"LassoSelectTool"}]},"id":"31643","type":"Toolbar"},{"attributes":{},"id":"31662","type":"LinearScale"},{"attributes":{},"id":"31676","type":"PanTool"},{"attributes":{},"id":"31622","type":"LinearScale"},{"attributes":{},"id":"31702","type":"Selection"},{"attributes":{"callback":null},"id":"31618","type":"DataRange1d"},{"attributes":{"overlay":{"id":"31703","type":"BoxAnnotation"}},"id":"31638","type":"BoxZoomTool"},{"attributes":{"callback":null},"id":"31620","type":"DataRange1d"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"31703","type":"BoxAnnotation"},{"attributes":{},"id":"31708","type":"BasicTickFormatter"},{"attributes":{"ticker":{"id":"31627","type":"BasicTicker"}},"id":"31630","type":"Grid"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#1f77b4"},"line_alpha":{"value":0.1},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y1"}},"id":"31693","type":"Circle"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"31704","type":"BoxAnnotation"},{"attributes":{},"id":"31627","type":"BasicTicker"},{"attributes":{"data_source":{"id":"31615","type":"ColumnDataSource"},"glyph":{"id":"31652","type":"Circle"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"31653","type":"Circle"},"selection_glyph":null,"view":{"id":"31655","type":"CDSView"}},"id":"31654","type":"GlyphRenderer"},{"attributes":{"fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"xs_units":"screen","ys_units":"screen"},"id":"31705","type":"PolyAnnotation"},{"attributes":{},"id":"31639","type":"ResetTool"},{"attributes":{},"id":"31640","type":"SaveTool"},{"attributes":{"formatter":{"id":"31710","type":"BasicTickFormatter"},"ticker":{"id":"31672","type":"BasicTicker"}},"id":"31671","type":"LinearAxis"},{"attributes":{"callback":null,"overlay":{"id":"31704","type":"BoxAnnotation"}},"id":"31641","type":"BoxSelectTool"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"31715","type":"BoxAnnotation"}],"root_ids":["31721"]},"title":"Bokeh Application","version":"1.3.2"}}';
                  var render_items = [{"docid":"3c077457-f4b6-470b-99e2-393348247ab0","roots":{"31721":"46c20642-7b06-4f83-90be-db5a8c1ad164"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        embed_document(root);
                        clearInterval(timer);
                      }
                      attempts++;
                      if (attempts > 100) {
                        console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        clearInterval(timer);
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {} // ensure no trailing comma for IE
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();