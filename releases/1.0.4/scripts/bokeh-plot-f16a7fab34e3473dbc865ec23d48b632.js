(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof (root._bokeh_onload_callbacks) === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) { callback() });
        }
        finally {
          delete root._bokeh_onload_callbacks
        }
        console.info("Bokeh: all callbacks have finished");
      }
    
      function load_libs(js_urls, callback) {
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.log("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.log("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = js_urls.length;
        for (var i = 0; i < js_urls.length; i++) {
          var url = js_urls[i];
          var s = document.createElement('script');
          s.src = url;
          s.async = false;
          s.onreadystatechange = s.onload = function() {
            root._bokeh_is_loading--;
            if (root._bokeh_is_loading === 0) {
              console.log("Bokeh: all BokehJS libraries loaded");
              run_callbacks()
            }
          };
          s.onerror = function() {
            console.warn("failed to load library " + url);
          };
          console.log("Bokeh: injecting script tag for BokehJS library: ", url);
          document.getElementsByTagName("head")[0].appendChild(s);
        }
      };var element = document.getElementById("936782de-1a68-4501-82db-6a111d677dbf");
      if (element == null) {
        console.log("Bokeh: ERROR: autoload.js configured with elementid '936782de-1a68-4501-82db-6a111d677dbf' but no matching script tag was found. ")
        return false;
      }
    
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-1.0.4.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-1.0.4.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-1.0.4.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-1.0.4.min.js"];
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"de128fde-2bb6-45d7-af58-71dc9f0737a6":{"roots":{"references":[{"attributes":{},"id":"23888","type":"Selection"},{"attributes":{"callback":null,"data":{"x":{"__ndarray__":"","dtype":"float64","shape":[500]},"y":{"__ndarray__":"","dtype":"float64","shape":[500]}},"selected":{"id":"23888","type":"Selection"},"selection_policy":{"id":"23887","type":"UnionRenderers"}},"id":"23834","type":"ColumnDataSource"},{"attributes":{},"id":"23857","type":"SaveTool"},{"attributes":{"dimension":1,"plot":{"id":"23835","subtype":"Figure","type":"Plot"},"ticker":{"id":"23850","type":"BasicTicker"}},"id":"23853","type":"Grid"},{"attributes":{"overlay":{"id":"23862","type":"BoxAnnotation"}},"id":"23856","type":"BoxZoomTool"},{"attributes":{"callback":{"id":"23874","type":"CustomJS"},"end":5,"start":-5,"step":0.1,"title":"Offset","value":0},"id":"23878","type":"Slider"},{"attributes":{},"id":"23842","type":"LinearScale"},{"attributes":{"plot":null,"text":""},"id":"23881","type":"Title"},{"attributes":{},"id":"23855","type":"WheelZoomTool"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"23854","type":"PanTool"},{"id":"23855","type":"WheelZoomTool"},{"id":"23856","type":"BoxZoomTool"},{"id":"23857","type":"SaveTool"},{"id":"23858","type":"ResetTool"},{"id":"23859","type":"HelpTool"}]},"id":"23860","type":"Toolbar"},{"attributes":{},"id":"23885","type":"BasicTickFormatter"},{"attributes":{"line_alpha":0.6,"line_color":"#1f77b4","line_width":3,"x":{"field":"x"},"y":{"field":"y"}},"id":"23870","type":"Line"},{"attributes":{},"id":"23859","type":"HelpTool"},{"attributes":{"source":{"id":"23834","type":"ColumnDataSource"}},"id":"23873","type":"CDSView"},{"attributes":{"data_source":{"id":"23834","type":"ColumnDataSource"},"glyph":{"id":"23870","type":"Line"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"23871","type":"Line"},"selection_glyph":null,"view":{"id":"23873","type":"CDSView"}},"id":"23872","type":"GlyphRenderer"},{"attributes":{"plot":{"id":"23835","subtype":"Figure","type":"Plot"},"ticker":{"id":"23845","type":"BasicTicker"}},"id":"23848","type":"Grid"},{"attributes":{"formatter":{"id":"23885","type":"BasicTickFormatter"},"plot":{"id":"23835","subtype":"Figure","type":"Plot"},"ticker":{"id":"23845","type":"BasicTicker"}},"id":"23844","type":"LinearAxis"},{"attributes":{},"id":"23850","type":"BasicTicker"},{"attributes":{},"id":"23883","type":"BasicTickFormatter"},{"attributes":{"below":[{"id":"23844","type":"LinearAxis"}],"left":[{"id":"23849","type":"LinearAxis"}],"plot_height":400,"plot_width":400,"renderers":[{"id":"23844","type":"LinearAxis"},{"id":"23848","type":"Grid"},{"id":"23849","type":"LinearAxis"},{"id":"23853","type":"Grid"},{"id":"23862","type":"BoxAnnotation"},{"id":"23872","type":"GlyphRenderer"}],"title":{"id":"23881","type":"Title"},"toolbar":{"id":"23860","type":"Toolbar"},"x_range":{"id":"23836","type":"DataRange1d"},"x_scale":{"id":"23840","type":"LinearScale"},"y_range":{"id":"23838","type":"Range1d"},"y_scale":{"id":"23842","type":"LinearScale"}},"id":"23835","subtype":"Figure","type":"Plot"},{"attributes":{"callback":null,"end":10,"start":-10},"id":"23838","type":"Range1d"},{"attributes":{},"id":"23887","type":"UnionRenderers"},{"attributes":{"callback":{"id":"23874","type":"CustomJS"},"end":6.4,"start":0,"step":0.1,"title":"Phase","value":0},"id":"23877","type":"Slider"},{"attributes":{"formatter":{"id":"23883","type":"BasicTickFormatter"},"plot":{"id":"23835","subtype":"Figure","type":"Plot"},"ticker":{"id":"23850","type":"BasicTicker"}},"id":"23849","type":"LinearAxis"},{"attributes":{},"id":"23845","type":"BasicTicker"},{"attributes":{},"id":"23840","type":"LinearScale"},{"attributes":{},"id":"23854","type":"PanTool"},{"attributes":{"children":[{"id":"23835","subtype":"Figure","type":"Plot"},{"id":"23879","type":"WidgetBox"}]},"id":"23880","type":"Row"},{"attributes":{"callback":{"id":"23874","type":"CustomJS"},"end":10,"start":0.1,"step":0.1,"title":"Amplitude","value":1},"id":"23875","type":"Slider"},{"attributes":{"children":[{"id":"23875","type":"Slider"},{"id":"23876","type":"Slider"},{"id":"23877","type":"Slider"},{"id":"23878","type":"Slider"}]},"id":"23879","type":"WidgetBox"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"23862","type":"BoxAnnotation"},{"attributes":{"callback":{"id":"23874","type":"CustomJS"},"end":10,"start":0.1,"step":0.1,"title":"Frequency","value":1},"id":"23876","type":"Slider"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","line_width":3,"x":{"field":"x"},"y":{"field":"y"}},"id":"23871","type":"Line"},{"attributes":{},"id":"23858","type":"ResetTool"},{"attributes":{"args":{"amp":{"id":"23875","type":"Slider"},"freq":{"id":"23876","type":"Slider"},"offset":{"id":"23878","type":"Slider"},"phase":{"id":"23877","type":"Slider"},"source":{"id":"23834","type":"ColumnDataSource"}},"code":"\\n    var data = source.data;\\n    var A = amp.value;\\n    var k = freq.value;\\n    var phi = phase.value;\\n    var B = offset.value;\\n    var x = data[&#x27;x&#x27;]\\n    var y = data[&#x27;y&#x27;]\\n    for (var i = 0; i &lt; x.length; i++) {\\n        y[i] = B + A*Math.sin(k*x[i]+phi);\\n    }\\n    source.change.emit();\\n"},"id":"23874","type":"CustomJS"},{"attributes":{"callback":null},"id":"23836","type":"DataRange1d"}],"root_ids":["23880"]},"title":"Bokeh Application","version":"1.0.4"}}';
                  var render_items = [{"docid":"de128fde-2bb6-45d7-af58-71dc9f0737a6","roots":{"23880":"936782de-1a68-4501-82db-6a111d677dbf"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        embed_document(root);
                        clearInterval(timer);
                      }
                      attempts++;
                      if (attempts > 100) {
                        console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        clearInterval(timer);
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
          console.log("Bokeh: injecting CSS: https://cdn.bokeh.org/bokeh/release/bokeh-1.0.4.min.css");
          Bokeh.embed.inject_css("https://cdn.bokeh.org/bokeh/release/bokeh-1.0.4.min.css");
          console.log("Bokeh: injecting CSS: https://cdn.bokeh.org/bokeh/release/bokeh-widgets-1.0.4.min.css");
          Bokeh.embed.inject_css("https://cdn.bokeh.org/bokeh/release/bokeh-widgets-1.0.4.min.css");
          console.log("Bokeh: injecting CSS: https://cdn.bokeh.org/bokeh/release/bokeh-tables-1.0.4.min.css");
          Bokeh.embed.inject_css("https://cdn.bokeh.org/bokeh/release/bokeh-tables-1.0.4.min.css");
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.log("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(js_urls, function() {
          console.log("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();