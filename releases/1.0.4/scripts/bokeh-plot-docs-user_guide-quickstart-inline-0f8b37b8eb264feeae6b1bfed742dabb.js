(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof (root._bokeh_onload_callbacks) === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) { callback() });
        }
        finally {
          delete root._bokeh_onload_callbacks
        }
        console.info("Bokeh: all callbacks have finished");
      }
    
      function load_libs(js_urls, callback) {
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.log("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.log("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = js_urls.length;
        for (var i = 0; i < js_urls.length; i++) {
          var url = js_urls[i];
          var s = document.createElement('script');
          s.src = url;
          s.async = false;
          s.onreadystatechange = s.onload = function() {
            root._bokeh_is_loading--;
            if (root._bokeh_is_loading === 0) {
              console.log("Bokeh: all BokehJS libraries loaded");
              run_callbacks()
            }
          };
          s.onerror = function() {
            console.warn("failed to load library " + url);
          };
          console.log("Bokeh: injecting script tag for BokehJS library: ", url);
          document.getElementsByTagName("head")[0].appendChild(s);
        }
      };var element = document.getElementById("912670b5-1def-481e-a43c-cad4992085e0");
      if (element == null) {
        console.log("Bokeh: ERROR: autoload.js configured with elementid '912670b5-1def-481e-a43c-cad4992085e0' but no matching script tag was found. ")
        return false;
      }
    
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-1.0.4.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-1.0.4.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-1.0.4.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-1.0.4.min.js"];
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"98627382-ac17-4a89-8f4f-8e665dc76f83":{"roots":{"references":[{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"36243","type":"BoxAnnotation"},{"attributes":{"fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"xs_units":"screen","ys_units":"screen"},"id":"36246","type":"PolyAnnotation"},{"attributes":{"data_source":{"id":"36162","type":"ColumnDataSource"},"glyph":{"id":"36251","type":"Circle"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"36252","type":"Circle"},"selection_glyph":null,"view":{"id":"36254","type":"CDSView"}},"id":"36253","type":"GlyphRenderer"},{"attributes":{"callback":null,"data":{"x":{"__ndarray__":"","dtype":"float64","shape":[300]},"y0":{"__ndarray__":"","dtype":"float64","shape":[300]},"y1":{"__ndarray__":"","dtype":"float64","shape":[300]}},"selected":{"id":"36261","type":"Selection"},"selection_policy":{"id":"36260","type":"UnionRenderers"}},"id":"36162","type":"ColumnDataSource"},{"attributes":{"below":[{"id":"36173","type":"LinearAxis"}],"left":[{"id":"36178","type":"LinearAxis"}],"plot_height":350,"plot_width":350,"renderers":[{"id":"36173","type":"LinearAxis"},{"id":"36177","type":"Grid"},{"id":"36178","type":"LinearAxis"},{"id":"36182","type":"Grid"},{"id":"36192","type":"BoxAnnotation"},{"id":"36197","type":"BoxAnnotation"},{"id":"36200","type":"PolyAnnotation"},{"id":"36207","type":"GlyphRenderer"}],"title":null,"toolbar":{"id":"36190","type":"Toolbar"},"toolbar_location":null,"x_range":{"id":"36165","type":"DataRange1d"},"x_scale":{"id":"36169","type":"LinearScale"},"y_range":{"id":"36167","type":"DataRange1d"},"y_scale":{"id":"36171","type":"LinearScale"}},"id":"36163","subtype":"Figure","type":"Plot"},{"attributes":{"source":{"id":"36162","type":"ColumnDataSource"}},"id":"36254","type":"CDSView"},{"attributes":{"children":[{"id":"36280","type":"ToolbarBox"},{"id":"36278","type":"Column"}]},"id":"36281","type":"Column"},{"attributes":{"children":[{"id":"36277","type":"Row"}]},"id":"36278","type":"Column"},{"attributes":{},"id":"36220","type":"BasicTicker"},{"attributes":{"children":[{"id":"36163","subtype":"Figure","type":"Plot"},{"id":"36209","subtype":"Figure","type":"Plot"}]},"id":"36277","type":"Row"},{"attributes":{"callback":null},"id":"36165","type":"DataRange1d"},{"attributes":{},"id":"36256","type":"BasicTickFormatter"},{"attributes":{"callback":null},"id":"36167","type":"DataRange1d"},{"attributes":{"plot":{"id":"36163","subtype":"Figure","type":"Plot"},"ticker":{"id":"36174","type":"BasicTicker"}},"id":"36177","type":"Grid"},{"attributes":{},"id":"36258","type":"BasicTickFormatter"},{"attributes":{},"id":"36174","type":"BasicTicker"},{"attributes":{},"id":"36260","type":"UnionRenderers"},{"attributes":{},"id":"36169","type":"LinearScale"},{"attributes":{},"id":"36261","type":"Selection"},{"attributes":{"formatter":{"id":"36258","type":"BasicTickFormatter"},"plot":{"id":"36163","subtype":"Figure","type":"Plot"},"ticker":{"id":"36174","type":"BasicTicker"}},"id":"36173","type":"LinearAxis"},{"attributes":{},"id":"36171","type":"LinearScale"},{"attributes":{"tools":[{"id":"36183","type":"PanTool"},{"id":"36184","type":"WheelZoomTool"},{"id":"36185","type":"BoxZoomTool"},{"id":"36186","type":"ResetTool"},{"id":"36187","type":"SaveTool"},{"id":"36188","type":"BoxSelectTool"},{"id":"36189","type":"LassoSelectTool"},{"id":"36229","type":"PanTool"},{"id":"36230","type":"WheelZoomTool"},{"id":"36231","type":"BoxZoomTool"},{"id":"36232","type":"ResetTool"},{"id":"36233","type":"SaveTool"},{"id":"36234","type":"BoxSelectTool"},{"id":"36235","type":"LassoSelectTool"}]},"id":"36279","type":"ProxyToolbar"},{"attributes":{"toolbar":{"id":"36279","type":"ProxyToolbar"},"toolbar_location":"above"},"id":"36280","type":"ToolbarBox"},{"attributes":{"dimension":1,"plot":{"id":"36163","subtype":"Figure","type":"Plot"},"ticker":{"id":"36179","type":"BasicTicker"}},"id":"36182","type":"Grid"},{"attributes":{"formatter":{"id":"36269","type":"BasicTickFormatter"},"plot":{"id":"36209","subtype":"Figure","type":"Plot"},"ticker":{"id":"36220","type":"BasicTicker"}},"id":"36219","type":"LinearAxis"},{"attributes":{"formatter":{"id":"36256","type":"BasicTickFormatter"},"plot":{"id":"36163","subtype":"Figure","type":"Plot"},"ticker":{"id":"36179","type":"BasicTicker"}},"id":"36178","type":"LinearAxis"},{"attributes":{},"id":"36179","type":"BasicTicker"},{"attributes":{},"id":"36183","type":"PanTool"},{"attributes":{},"id":"36233","type":"SaveTool"},{"attributes":{"fill_color":{"value":"#1f77b4"},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y0"}},"id":"36205","type":"Circle"},{"attributes":{},"id":"36269","type":"BasicTickFormatter"},{"attributes":{"overlay":{"id":"36238","type":"BoxAnnotation"}},"id":"36231","type":"BoxZoomTool"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"36183","type":"PanTool"},{"id":"36184","type":"WheelZoomTool"},{"id":"36185","type":"BoxZoomTool"},{"id":"36186","type":"ResetTool"},{"id":"36187","type":"SaveTool"},{"id":"36188","type":"BoxSelectTool"},{"id":"36189","type":"LassoSelectTool"}]},"id":"36190","type":"Toolbar"},{"attributes":{},"id":"36229","type":"PanTool"},{"attributes":{},"id":"36184","type":"WheelZoomTool"},{"attributes":{"callback":null,"overlay":{"id":"36246","type":"PolyAnnotation"}},"id":"36235","type":"LassoSelectTool"},{"attributes":{"overlay":{"id":"36192","type":"BoxAnnotation"}},"id":"36185","type":"BoxZoomTool"},{"attributes":{"dimension":1,"plot":{"id":"36209","subtype":"Figure","type":"Plot"},"ticker":{"id":"36225","type":"BasicTicker"}},"id":"36228","type":"Grid"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#1f77b4"},"line_alpha":{"value":0.1},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y1"}},"id":"36252","type":"Circle"},{"attributes":{},"id":"36186","type":"ResetTool"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"36192","type":"BoxAnnotation"},{"attributes":{},"id":"36187","type":"SaveTool"},{"attributes":{"callback":null,"overlay":{"id":"36243","type":"BoxAnnotation"}},"id":"36234","type":"BoxSelectTool"},{"attributes":{"plot":{"id":"36209","subtype":"Figure","type":"Plot"},"ticker":{"id":"36220","type":"BasicTicker"}},"id":"36223","type":"Grid"},{"attributes":{"callback":null,"overlay":{"id":"36197","type":"BoxAnnotation"}},"id":"36188","type":"BoxSelectTool"},{"attributes":{},"id":"36232","type":"ResetTool"},{"attributes":{"callback":null,"overlay":{"id":"36200","type":"PolyAnnotation"}},"id":"36189","type":"LassoSelectTool"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#1f77b4"},"line_alpha":{"value":0.1},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y0"}},"id":"36206","type":"Circle"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"36197","type":"BoxAnnotation"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"36238","type":"BoxAnnotation"},{"attributes":{"fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"xs_units":"screen","ys_units":"screen"},"id":"36200","type":"PolyAnnotation"},{"attributes":{"callback":null},"id":"36213","type":"DataRange1d"},{"attributes":{"data_source":{"id":"36162","type":"ColumnDataSource"},"glyph":{"id":"36205","type":"Circle"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"36206","type":"Circle"},"selection_glyph":null,"view":{"id":"36208","type":"CDSView"}},"id":"36207","type":"GlyphRenderer"},{"attributes":{},"id":"36230","type":"WheelZoomTool"},{"attributes":{"source":{"id":"36162","type":"ColumnDataSource"}},"id":"36208","type":"CDSView"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"36229","type":"PanTool"},{"id":"36230","type":"WheelZoomTool"},{"id":"36231","type":"BoxZoomTool"},{"id":"36232","type":"ResetTool"},{"id":"36233","type":"SaveTool"},{"id":"36234","type":"BoxSelectTool"},{"id":"36235","type":"LassoSelectTool"}]},"id":"36236","type":"Toolbar"},{"attributes":{"below":[{"id":"36219","type":"LinearAxis"}],"left":[{"id":"36224","type":"LinearAxis"}],"plot_height":350,"plot_width":350,"renderers":[{"id":"36219","type":"LinearAxis"},{"id":"36223","type":"Grid"},{"id":"36224","type":"LinearAxis"},{"id":"36228","type":"Grid"},{"id":"36238","type":"BoxAnnotation"},{"id":"36243","type":"BoxAnnotation"},{"id":"36246","type":"PolyAnnotation"},{"id":"36253","type":"GlyphRenderer"}],"title":null,"toolbar":{"id":"36236","type":"Toolbar"},"toolbar_location":null,"x_range":{"id":"36211","type":"DataRange1d"},"x_scale":{"id":"36215","type":"LinearScale"},"y_range":{"id":"36213","type":"DataRange1d"},"y_scale":{"id":"36217","type":"LinearScale"}},"id":"36209","subtype":"Figure","type":"Plot"},{"attributes":{"fill_color":{"value":"#1f77b4"},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y1"}},"id":"36251","type":"Circle"},{"attributes":{"formatter":{"id":"36267","type":"BasicTickFormatter"},"plot":{"id":"36209","subtype":"Figure","type":"Plot"},"ticker":{"id":"36225","type":"BasicTicker"}},"id":"36224","type":"LinearAxis"},{"attributes":{},"id":"36267","type":"BasicTickFormatter"},{"attributes":{"callback":null},"id":"36211","type":"DataRange1d"},{"attributes":{},"id":"36215","type":"LinearScale"},{"attributes":{},"id":"36225","type":"BasicTicker"},{"attributes":{},"id":"36217","type":"LinearScale"}],"root_ids":["36281"]},"title":"Bokeh Application","version":"1.0.4"}}';
                  var render_items = [{"docid":"98627382-ac17-4a89-8f4f-8e665dc76f83","roots":{"36281":"912670b5-1def-481e-a43c-cad4992085e0"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        embed_document(root);
                        clearInterval(timer);
                      }
                      attempts++;
                      if (attempts > 100) {
                        console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        clearInterval(timer);
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
          console.log("Bokeh: injecting CSS: https://cdn.bokeh.org/bokeh/release/bokeh-1.0.4.min.css");
          Bokeh.embed.inject_css("https://cdn.bokeh.org/bokeh/release/bokeh-1.0.4.min.css");
          console.log("Bokeh: injecting CSS: https://cdn.bokeh.org/bokeh/release/bokeh-widgets-1.0.4.min.css");
          Bokeh.embed.inject_css("https://cdn.bokeh.org/bokeh/release/bokeh-widgets-1.0.4.min.css");
          console.log("Bokeh: injecting CSS: https://cdn.bokeh.org/bokeh/release/bokeh-tables-1.0.4.min.css");
          Bokeh.embed.inject_css("https://cdn.bokeh.org/bokeh/release/bokeh-tables-1.0.4.min.css");
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.log("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(js_urls, function() {
          console.log("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();