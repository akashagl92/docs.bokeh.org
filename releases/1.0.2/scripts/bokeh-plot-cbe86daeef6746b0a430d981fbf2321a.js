(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof (root._bokeh_onload_callbacks) === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) { callback() });
        }
        finally {
          delete root._bokeh_onload_callbacks
        }
        console.info("Bokeh: all callbacks have finished");
      }
    
      function load_libs(js_urls, callback) {
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.log("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.log("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = js_urls.length;
        for (var i = 0; i < js_urls.length; i++) {
          var url = js_urls[i];
          var s = document.createElement('script');
          s.src = url;
          s.async = false;
          s.onreadystatechange = s.onload = function() {
            root._bokeh_is_loading--;
            if (root._bokeh_is_loading === 0) {
              console.log("Bokeh: all BokehJS libraries loaded");
              run_callbacks()
            }
          };
          s.onerror = function() {
            console.warn("failed to load library " + url);
          };
          console.log("Bokeh: injecting script tag for BokehJS library: ", url);
          document.getElementsByTagName("head")[0].appendChild(s);
        }
      };var element = document.getElementById("7f0e5fef-df37-4371-8ca7-da398e03f757");
      if (element == null) {
        console.log("Bokeh: ERROR: autoload.js configured with elementid '7f0e5fef-df37-4371-8ca7-da398e03f757' but no matching script tag was found. ")
        return false;
      }
    
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-1.0.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-1.0.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-1.0.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-1.0.2.min.js"];
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"de381748-9abd-46af-9d98-6182533cd32f":{"roots":{"references":[{"attributes":{},"id":"23842","type":"BasicTicker"},{"attributes":{"below":[{"id":"23841","type":"LinearAxis"}],"left":[{"id":"23846","type":"LinearAxis"}],"plot_height":400,"plot_width":400,"renderers":[{"id":"23841","type":"LinearAxis"},{"id":"23845","type":"Grid"},{"id":"23846","type":"LinearAxis"},{"id":"23850","type":"Grid"},{"id":"23859","type":"BoxAnnotation"},{"id":"23869","type":"GlyphRenderer"}],"title":{"id":"23879","type":"Title"},"toolbar":{"id":"23857","type":"Toolbar"},"x_range":{"id":"23833","type":"DataRange1d"},"x_scale":{"id":"23837","type":"LinearScale"},"y_range":{"id":"23835","type":"Range1d"},"y_scale":{"id":"23839","type":"LinearScale"}},"id":"23832","subtype":"Figure","type":"Plot"},{"attributes":{},"id":"23886","type":"Selection"},{"attributes":{},"id":"23881","type":"BasicTickFormatter"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","line_width":3,"x":{"field":"x"},"y":{"field":"y"}},"id":"23868","type":"Line"},{"attributes":{},"id":"23855","type":"ResetTool"},{"attributes":{"args":{"amp":{"id":"23872","type":"Slider"},"freq":{"id":"23873","type":"Slider"},"offset":{"id":"23875","type":"Slider"},"phase":{"id":"23874","type":"Slider"},"source":{"id":"23831","type":"ColumnDataSource"}},"code":"\\n    var data = source.data;\\n    var A = amp.value;\\n    var k = freq.value;\\n    var phi = phase.value;\\n    var B = offset.value;\\n    var x = data[&#x27;x&#x27;]\\n    var y = data[&#x27;y&#x27;]\\n    for (var i = 0; i &lt; x.length; i++) {\\n        y[i] = B + A*Math.sin(k*x[i]+phi);\\n    }\\n    source.change.emit();\\n"},"id":"23871","type":"CustomJS"},{"attributes":{},"id":"23883","type":"BasicTickFormatter"},{"attributes":{"dimension":1,"plot":{"id":"23832","subtype":"Figure","type":"Plot"},"ticker":{"id":"23847","type":"BasicTicker"}},"id":"23850","type":"Grid"},{"attributes":{"callback":null},"id":"23833","type":"DataRange1d"},{"attributes":{"formatter":{"id":"23883","type":"BasicTickFormatter"},"plot":{"id":"23832","subtype":"Figure","type":"Plot"},"ticker":{"id":"23847","type":"BasicTicker"}},"id":"23846","type":"LinearAxis"},{"attributes":{"source":{"id":"23831","type":"ColumnDataSource"}},"id":"23870","type":"CDSView"},{"attributes":{},"id":"23839","type":"LinearScale"},{"attributes":{},"id":"23856","type":"HelpTool"},{"attributes":{"callback":{"id":"23871","type":"CustomJS"},"end":5,"start":-5,"step":0.1,"title":"Offset","value":0},"id":"23875","type":"Slider"},{"attributes":{},"id":"23851","type":"PanTool"},{"attributes":{},"id":"23854","type":"SaveTool"},{"attributes":{"children":[{"id":"23872","type":"Slider"},{"id":"23873","type":"Slider"},{"id":"23874","type":"Slider"},{"id":"23875","type":"Slider"}]},"id":"23876","type":"WidgetBox"},{"attributes":{"callback":{"id":"23871","type":"CustomJS"},"end":10,"start":0.1,"step":0.1,"title":"Amplitude","value":1},"id":"23872","type":"Slider"},{"attributes":{"overlay":{"id":"23859","type":"BoxAnnotation"}},"id":"23853","type":"BoxZoomTool"},{"attributes":{"formatter":{"id":"23881","type":"BasicTickFormatter"},"plot":{"id":"23832","subtype":"Figure","type":"Plot"},"ticker":{"id":"23842","type":"BasicTicker"}},"id":"23841","type":"LinearAxis"},{"attributes":{"children":[{"id":"23832","subtype":"Figure","type":"Plot"},{"id":"23876","type":"WidgetBox"}]},"id":"23877","type":"Row"},{"attributes":{},"id":"23837","type":"LinearScale"},{"attributes":{},"id":"23847","type":"BasicTicker"},{"attributes":{"callback":null,"end":10,"start":-10},"id":"23835","type":"Range1d"},{"attributes":{},"id":"23885","type":"UnionRenderers"},{"attributes":{"plot":{"id":"23832","subtype":"Figure","type":"Plot"},"ticker":{"id":"23842","type":"BasicTicker"}},"id":"23845","type":"Grid"},{"attributes":{"callback":{"id":"23871","type":"CustomJS"},"end":10,"start":0.1,"step":0.1,"title":"Frequency","value":1},"id":"23873","type":"Slider"},{"attributes":{"line_alpha":0.6,"line_color":"#1f77b4","line_width":3,"x":{"field":"x"},"y":{"field":"y"}},"id":"23867","type":"Line"},{"attributes":{"plot":null,"text":""},"id":"23879","type":"Title"},{"attributes":{"callback":{"id":"23871","type":"CustomJS"},"end":6.4,"start":0,"step":0.1,"title":"Phase","value":0},"id":"23874","type":"Slider"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"23859","type":"BoxAnnotation"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"23851","type":"PanTool"},{"id":"23852","type":"WheelZoomTool"},{"id":"23853","type":"BoxZoomTool"},{"id":"23854","type":"SaveTool"},{"id":"23855","type":"ResetTool"},{"id":"23856","type":"HelpTool"}]},"id":"23857","type":"Toolbar"},{"attributes":{"callback":null,"data":{"x":{"__ndarray__":"","dtype":"float64","shape":[500]},"y":{"__ndarray__":"","dtype":"float64","shape":[500]}},"selected":{"id":"23886","type":"Selection"},"selection_policy":{"id":"23885","type":"UnionRenderers"}},"id":"23831","type":"ColumnDataSource"},{"attributes":{"data_source":{"id":"23831","type":"ColumnDataSource"},"glyph":{"id":"23867","type":"Line"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"23868","type":"Line"},"selection_glyph":null,"view":{"id":"23870","type":"CDSView"}},"id":"23869","type":"GlyphRenderer"},{"attributes":{},"id":"23852","type":"WheelZoomTool"}],"root_ids":["23877"]},"title":"Bokeh Application","version":"1.0.2"}}';
                  var render_items = [{"docid":"de381748-9abd-46af-9d98-6182533cd32f","roots":{"23877":"7f0e5fef-df37-4371-8ca7-da398e03f757"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        embed_document(root);
                        clearInterval(timer);
                      }
                      attempts++;
                      if (attempts > 100) {
                        console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        clearInterval(timer);
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
          console.log("Bokeh: injecting CSS: https://cdn.bokeh.org/bokeh/release/bokeh-1.0.2.min.css");
          Bokeh.embed.inject_css("https://cdn.bokeh.org/bokeh/release/bokeh-1.0.2.min.css");
          console.log("Bokeh: injecting CSS: https://cdn.bokeh.org/bokeh/release/bokeh-widgets-1.0.2.min.css");
          Bokeh.embed.inject_css("https://cdn.bokeh.org/bokeh/release/bokeh-widgets-1.0.2.min.css");
          console.log("Bokeh: injecting CSS: https://cdn.bokeh.org/bokeh/release/bokeh-tables-1.0.2.min.css");
          Bokeh.embed.inject_css("https://cdn.bokeh.org/bokeh/release/bokeh-tables-1.0.2.min.css");
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.log("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(js_urls, function() {
          console.log("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();