(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof (root._bokeh_onload_callbacks) === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) { callback() });
        }
        finally {
          delete root._bokeh_onload_callbacks
        }
        console.info("Bokeh: all callbacks have finished");
      }
    
      function load_libs(js_urls, callback) {
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.log("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.log("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = js_urls.length;
        for (var i = 0; i < js_urls.length; i++) {
          var url = js_urls[i];
          var s = document.createElement('script');
          s.src = url;
          s.async = false;
          s.onreadystatechange = s.onload = function() {
            root._bokeh_is_loading--;
            if (root._bokeh_is_loading === 0) {
              console.log("Bokeh: all BokehJS libraries loaded");
              run_callbacks()
            }
          };
          s.onerror = function() {
            console.warn("failed to load library " + url);
          };
          console.log("Bokeh: injecting script tag for BokehJS library: ", url);
          document.getElementsByTagName("head")[0].appendChild(s);
        }
      };var element = document.getElementById("da94c1cd-2eb7-48aa-bc37-0ac49add7301");
      if (element == null) {
        console.log("Bokeh: ERROR: autoload.js configured with elementid 'da94c1cd-2eb7-48aa-bc37-0ac49add7301' but no matching script tag was found. ")
        return false;
      }
    
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-1.0.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-1.0.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-1.0.2.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-1.0.2.min.js"];
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"5336ce15-e38b-4e21-b0cb-518dd11d9667":{"roots":{"references":[{"attributes":{},"id":"36219","type":"ResetTool"},{"attributes":{"below":[{"id":"36160","type":"LinearAxis"}],"left":[{"id":"36165","type":"LinearAxis"}],"plot_height":350,"plot_width":350,"renderers":[{"id":"36160","type":"LinearAxis"},{"id":"36164","type":"Grid"},{"id":"36165","type":"LinearAxis"},{"id":"36169","type":"Grid"},{"id":"36179","type":"BoxAnnotation"},{"id":"36184","type":"BoxAnnotation"},{"id":"36187","type":"PolyAnnotation"},{"id":"36194","type":"GlyphRenderer"}],"title":null,"toolbar":{"id":"36177","type":"Toolbar"},"toolbar_location":null,"x_range":{"id":"36152","type":"DataRange1d"},"x_scale":{"id":"36156","type":"LinearScale"},"y_range":{"id":"36154","type":"DataRange1d"},"y_scale":{"id":"36158","type":"LinearScale"}},"id":"36150","subtype":"Figure","type":"Plot"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"36170","type":"PanTool"},{"id":"36171","type":"WheelZoomTool"},{"id":"36172","type":"BoxZoomTool"},{"id":"36173","type":"ResetTool"},{"id":"36174","type":"SaveTool"},{"id":"36175","type":"BoxSelectTool"},{"id":"36176","type":"LassoSelectTool"}]},"id":"36177","type":"Toolbar"},{"attributes":{},"id":"36212","type":"BasicTicker"},{"attributes":{},"id":"36217","type":"WheelZoomTool"},{"attributes":{"overlay":{"id":"36179","type":"BoxAnnotation"}},"id":"36172","type":"BoxZoomTool"},{"attributes":{},"id":"36173","type":"ResetTool"},{"attributes":{"data_source":{"id":"36149","type":"ColumnDataSource"},"glyph":{"id":"36238","type":"Circle"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"36239","type":"Circle"},"selection_glyph":null,"view":{"id":"36241","type":"CDSView"}},"id":"36240","type":"GlyphRenderer"},{"attributes":{"children":[{"id":"36150","subtype":"Figure","type":"Plot"},{"id":"36196","subtype":"Figure","type":"Plot"}]},"id":"36264","type":"Row"},{"attributes":{"callback":null,"overlay":{"id":"36187","type":"PolyAnnotation"}},"id":"36176","type":"LassoSelectTool"},{"attributes":{"overlay":{"id":"36225","type":"BoxAnnotation"}},"id":"36218","type":"BoxZoomTool"},{"attributes":{"callback":null},"id":"36198","type":"DataRange1d"},{"attributes":{},"id":"36166","type":"BasicTicker"},{"attributes":{"fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"xs_units":"screen","ys_units":"screen"},"id":"36233","type":"PolyAnnotation"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"36230","type":"BoxAnnotation"},{"attributes":{"formatter":{"id":"36246","type":"BasicTickFormatter"},"plot":{"id":"36150","subtype":"Figure","type":"Plot"},"ticker":{"id":"36166","type":"BasicTicker"}},"id":"36165","type":"LinearAxis"},{"attributes":{},"id":"36171","type":"WheelZoomTool"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"36184","type":"BoxAnnotation"},{"attributes":{},"id":"36246","type":"BasicTickFormatter"},{"attributes":{"callback":null,"overlay":{"id":"36184","type":"BoxAnnotation"}},"id":"36175","type":"BoxSelectTool"},{"attributes":{"children":[{"id":"36267","type":"ToolbarBox"},{"id":"36265","type":"Column"}]},"id":"36268","type":"Column"},{"attributes":{"callback":null},"id":"36154","type":"DataRange1d"},{"attributes":{},"id":"36244","type":"BasicTickFormatter"},{"attributes":{},"id":"36257","type":"BasicTickFormatter"},{"attributes":{"source":{"id":"36149","type":"ColumnDataSource"}},"id":"36241","type":"CDSView"},{"attributes":{"below":[{"id":"36206","type":"LinearAxis"}],"left":[{"id":"36211","type":"LinearAxis"}],"plot_height":350,"plot_width":350,"renderers":[{"id":"36206","type":"LinearAxis"},{"id":"36210","type":"Grid"},{"id":"36211","type":"LinearAxis"},{"id":"36215","type":"Grid"},{"id":"36225","type":"BoxAnnotation"},{"id":"36230","type":"BoxAnnotation"},{"id":"36233","type":"PolyAnnotation"},{"id":"36240","type":"GlyphRenderer"}],"title":null,"toolbar":{"id":"36223","type":"Toolbar"},"toolbar_location":null,"x_range":{"id":"36198","type":"DataRange1d"},"x_scale":{"id":"36202","type":"LinearScale"},"y_range":{"id":"36200","type":"DataRange1d"},"y_scale":{"id":"36204","type":"LinearScale"}},"id":"36196","subtype":"Figure","type":"Plot"},{"attributes":{"callback":null,"data":{"x":{"__ndarray__":"","dtype":"float64","shape":[300]},"y0":{"__ndarray__":"","dtype":"float64","shape":[300]},"y1":{"__ndarray__":"","dtype":"float64","shape":[300]}},"selected":{"id":"36251","type":"Selection"},"selection_policy":{"id":"36250","type":"UnionRenderers"}},"id":"36149","type":"ColumnDataSource"},{"attributes":{"dimension":1,"plot":{"id":"36150","subtype":"Figure","type":"Plot"},"ticker":{"id":"36166","type":"BasicTicker"}},"id":"36169","type":"Grid"},{"attributes":{"children":[{"id":"36264","type":"Row"}]},"id":"36265","type":"Column"},{"attributes":{"plot":{"id":"36150","subtype":"Figure","type":"Plot"},"ticker":{"id":"36161","type":"BasicTicker"}},"id":"36164","type":"Grid"},{"attributes":{"callback":null,"overlay":{"id":"36233","type":"PolyAnnotation"}},"id":"36222","type":"LassoSelectTool"},{"attributes":{"formatter":{"id":"36244","type":"BasicTickFormatter"},"plot":{"id":"36150","subtype":"Figure","type":"Plot"},"ticker":{"id":"36161","type":"BasicTicker"}},"id":"36160","type":"LinearAxis"},{"attributes":{"data_source":{"id":"36149","type":"ColumnDataSource"},"glyph":{"id":"36192","type":"Circle"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"36193","type":"Circle"},"selection_glyph":null,"view":{"id":"36195","type":"CDSView"}},"id":"36194","type":"GlyphRenderer"},{"attributes":{},"id":"36204","type":"LinearScale"},{"attributes":{},"id":"36220","type":"SaveTool"},{"attributes":{"toolbar":{"id":"36266","type":"ProxyToolbar"},"toolbar_location":"above"},"id":"36267","type":"ToolbarBox"},{"attributes":{"tools":[{"id":"36170","type":"PanTool"},{"id":"36171","type":"WheelZoomTool"},{"id":"36172","type":"BoxZoomTool"},{"id":"36173","type":"ResetTool"},{"id":"36174","type":"SaveTool"},{"id":"36175","type":"BoxSelectTool"},{"id":"36176","type":"LassoSelectTool"},{"id":"36216","type":"PanTool"},{"id":"36217","type":"WheelZoomTool"},{"id":"36218","type":"BoxZoomTool"},{"id":"36219","type":"ResetTool"},{"id":"36220","type":"SaveTool"},{"id":"36221","type":"BoxSelectTool"},{"id":"36222","type":"LassoSelectTool"}]},"id":"36266","type":"ProxyToolbar"},{"attributes":{},"id":"36170","type":"PanTool"},{"attributes":{},"id":"36202","type":"LinearScale"},{"attributes":{},"id":"36158","type":"LinearScale"},{"attributes":{},"id":"36251","type":"Selection"},{"attributes":{"callback":null},"id":"36200","type":"DataRange1d"},{"attributes":{},"id":"36156","type":"LinearScale"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"36216","type":"PanTool"},{"id":"36217","type":"WheelZoomTool"},{"id":"36218","type":"BoxZoomTool"},{"id":"36219","type":"ResetTool"},{"id":"36220","type":"SaveTool"},{"id":"36221","type":"BoxSelectTool"},{"id":"36222","type":"LassoSelectTool"}]},"id":"36223","type":"Toolbar"},{"attributes":{"formatter":{"id":"36257","type":"BasicTickFormatter"},"plot":{"id":"36196","subtype":"Figure","type":"Plot"},"ticker":{"id":"36212","type":"BasicTicker"}},"id":"36211","type":"LinearAxis"},{"attributes":{"plot":{"id":"36196","subtype":"Figure","type":"Plot"},"ticker":{"id":"36207","type":"BasicTicker"}},"id":"36210","type":"Grid"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"36179","type":"BoxAnnotation"},{"attributes":{"dimension":1,"plot":{"id":"36196","subtype":"Figure","type":"Plot"},"ticker":{"id":"36212","type":"BasicTicker"}},"id":"36215","type":"Grid"},{"attributes":{},"id":"36161","type":"BasicTicker"},{"attributes":{},"id":"36174","type":"SaveTool"},{"attributes":{},"id":"36250","type":"UnionRenderers"},{"attributes":{},"id":"36216","type":"PanTool"},{"attributes":{},"id":"36255","type":"BasicTickFormatter"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#1f77b4"},"line_alpha":{"value":0.1},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y1"}},"id":"36239","type":"Circle"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"36225","type":"BoxAnnotation"},{"attributes":{},"id":"36207","type":"BasicTicker"},{"attributes":{"formatter":{"id":"36255","type":"BasicTickFormatter"},"plot":{"id":"36196","subtype":"Figure","type":"Plot"},"ticker":{"id":"36207","type":"BasicTicker"}},"id":"36206","type":"LinearAxis"},{"attributes":{"callback":null},"id":"36152","type":"DataRange1d"},{"attributes":{"fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"xs_units":"screen","ys_units":"screen"},"id":"36187","type":"PolyAnnotation"},{"attributes":{"fill_color":{"value":"#1f77b4"},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y1"}},"id":"36238","type":"Circle"},{"attributes":{"fill_color":{"value":"#1f77b4"},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y0"}},"id":"36192","type":"Circle"},{"attributes":{"callback":null,"overlay":{"id":"36230","type":"BoxAnnotation"}},"id":"36221","type":"BoxSelectTool"},{"attributes":{"source":{"id":"36149","type":"ColumnDataSource"}},"id":"36195","type":"CDSView"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#1f77b4"},"line_alpha":{"value":0.1},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y0"}},"id":"36193","type":"Circle"}],"root_ids":["36268"]},"title":"Bokeh Application","version":"1.0.2"}}';
                  var render_items = [{"docid":"5336ce15-e38b-4e21-b0cb-518dd11d9667","roots":{"36268":"da94c1cd-2eb7-48aa-bc37-0ac49add7301"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        embed_document(root);
                        clearInterval(timer);
                      }
                      attempts++;
                      if (attempts > 100) {
                        console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        clearInterval(timer);
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
          console.log("Bokeh: injecting CSS: https://cdn.bokeh.org/bokeh/release/bokeh-1.0.2.min.css");
          Bokeh.embed.inject_css("https://cdn.bokeh.org/bokeh/release/bokeh-1.0.2.min.css");
          console.log("Bokeh: injecting CSS: https://cdn.bokeh.org/bokeh/release/bokeh-widgets-1.0.2.min.css");
          Bokeh.embed.inject_css("https://cdn.bokeh.org/bokeh/release/bokeh-widgets-1.0.2.min.css");
          console.log("Bokeh: injecting CSS: https://cdn.bokeh.org/bokeh/release/bokeh-tables-1.0.2.min.css");
          Bokeh.embed.inject_css("https://cdn.bokeh.org/bokeh/release/bokeh-tables-1.0.2.min.css");
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.log("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(js_urls, function() {
          console.log("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();