(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error() {
          console.error("failed to load " + url);
        }
    
        for (var i = 0; i < css_urls.length; i++) {
          var url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error;
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        for (var i = 0; i < js_urls.length; i++) {
          var url = js_urls[i];
          var element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error;
          element.async = false;
          element.src = url;
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };var element = document.getElementById("3af0a73d-c649-4eb1-a75d-b53420d6b763");
      if (element == null) {
        console.error("Bokeh: ERROR: autoload.js configured with elementid '3af0a73d-c649-4eb1-a75d-b53420d6b763' but no matching script tag was found. ")
        return false;
      }
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      var js_urls = ["https://cdn.pydata.org/bokeh/release/bokeh-1.3.0.min.js", "https://cdn.pydata.org/bokeh/release/bokeh-widgets-1.3.0.min.js", "https://cdn.pydata.org/bokeh/release/bokeh-tables-1.3.0.min.js", "https://cdn.pydata.org/bokeh/release/bokeh-gl-1.3.0.min.js", "https://cdn.pydata.org/bokeh/release/bokeh-api-1.3.0.min.js"];
      var css_urls = [];
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"83ebec1d-21f4-46cf-a994-bfe37d46e3af":{"roots":{"references":[{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"31711","type":"BoxAnnotation"},{"attributes":{},"id":"31631","type":"BasicTicker"},{"attributes":{"formatter":{"id":"31699","type":"BasicTickFormatter"},"ticker":{"id":"31631","type":"BasicTicker"}},"id":"31630","type":"LinearAxis"},{"attributes":{},"id":"31676","type":"WheelZoomTool"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"31712","type":"BoxAnnotation"},{"attributes":{"fill_color":{"value":"#1f77b4"},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y1"}},"id":"31691","type":"Circle"},{"attributes":{"overlay":{"id":"31711","type":"BoxAnnotation"}},"id":"31677","type":"BoxZoomTool"},{"attributes":{},"id":"31639","type":"SaveTool"},{"attributes":{"fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"xs_units":"screen","ys_units":"screen"},"id":"31713","type":"PolyAnnotation"},{"attributes":{},"id":"31626","type":"BasicTicker"},{"attributes":{},"id":"31678","type":"ResetTool"},{"attributes":{"tools":[{"id":"31635","type":"PanTool"},{"id":"31636","type":"WheelZoomTool"},{"id":"31637","type":"BoxZoomTool"},{"id":"31638","type":"ResetTool"},{"id":"31639","type":"SaveTool"},{"id":"31640","type":"BoxSelectTool"},{"id":"31641","type":"LassoSelectTool"},{"id":"31675","type":"PanTool"},{"id":"31676","type":"WheelZoomTool"},{"id":"31677","type":"BoxZoomTool"},{"id":"31678","type":"ResetTool"},{"id":"31679","type":"SaveTool"},{"id":"31680","type":"BoxSelectTool"},{"id":"31681","type":"LassoSelectTool"}]},"id":"31718","type":"ProxyToolbar"},{"attributes":{},"id":"31679","type":"SaveTool"},{"attributes":{"overlay":{"id":"31700","type":"BoxAnnotation"}},"id":"31637","type":"BoxZoomTool"},{"attributes":{"toolbar":{"id":"31718","type":"ProxyToolbar"},"toolbar_location":"above"},"id":"31719","type":"ToolbarBox"},{"attributes":{},"id":"31635","type":"PanTool"},{"attributes":{"callback":null,"overlay":{"id":"31712","type":"BoxAnnotation"}},"id":"31680","type":"BoxSelectTool"},{"attributes":{"children":[{"id":"31719","type":"ToolbarBox"},{"id":"31717","type":"GridBox"}]},"id":"31720","type":"Column"},{"attributes":{"formatter":{"id":"31697","type":"BasicTickFormatter"},"ticker":{"id":"31626","type":"BasicTicker"}},"id":"31625","type":"LinearAxis"},{"attributes":{"fill_color":{"value":"#1f77b4"},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y0"}},"id":"31651","type":"Circle"},{"attributes":{"callback":null,"overlay":{"id":"31713","type":"PolyAnnotation"}},"id":"31681","type":"LassoSelectTool"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"31635","type":"PanTool"},{"id":"31636","type":"WheelZoomTool"},{"id":"31637","type":"BoxZoomTool"},{"id":"31638","type":"ResetTool"},{"id":"31639","type":"SaveTool"},{"id":"31640","type":"BoxSelectTool"},{"id":"31641","type":"LassoSelectTool"}]},"id":"31642","type":"Toolbar"},{"attributes":{},"id":"31638","type":"ResetTool"},{"attributes":{},"id":"31675","type":"PanTool"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"31675","type":"PanTool"},{"id":"31676","type":"WheelZoomTool"},{"id":"31677","type":"BoxZoomTool"},{"id":"31678","type":"ResetTool"},{"id":"31679","type":"SaveTool"},{"id":"31680","type":"BoxSelectTool"},{"id":"31681","type":"LassoSelectTool"}]},"id":"31682","type":"Toolbar"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#1f77b4"},"line_alpha":{"value":0.1},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y0"}},"id":"31652","type":"Circle"},{"attributes":{"data_source":{"id":"31614","type":"ColumnDataSource"},"glyph":{"id":"31691","type":"Circle"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"31692","type":"Circle"},"selection_glyph":null,"view":{"id":"31694","type":"CDSView"}},"id":"31693","type":"GlyphRenderer"},{"attributes":{"formatter":{"id":"31708","type":"BasicTickFormatter"},"ticker":{"id":"31666","type":"BasicTicker"}},"id":"31665","type":"LinearAxis"},{"attributes":{},"id":"31621","type":"LinearScale"},{"attributes":{"callback":null,"overlay":{"id":"31701","type":"BoxAnnotation"}},"id":"31640","type":"BoxSelectTool"},{"attributes":{},"id":"31666","type":"BasicTicker"},{"attributes":{"source":{"id":"31614","type":"ColumnDataSource"}},"id":"31694","type":"CDSView"},{"attributes":{"callback":null,"overlay":{"id":"31702","type":"PolyAnnotation"}},"id":"31641","type":"LassoSelectTool"},{"attributes":{"callback":null,"data":{"x":{"__ndarray__":"","dtype":"float64","shape":[300]},"y0":{"__ndarray__":"","dtype":"float64","shape":[300]},"y1":{"__ndarray__":"","dtype":"float64","shape":[300]}},"selected":{"id":"31704","type":"Selection"},"selection_policy":{"id":"31703","type":"UnionRenderers"}},"id":"31614","type":"ColumnDataSource"},{"attributes":{},"id":"31671","type":"BasicTicker"},{"attributes":{"below":[{"id":"31625","type":"LinearAxis"}],"center":[{"id":"31629","type":"Grid"},{"id":"31634","type":"Grid"}],"left":[{"id":"31630","type":"LinearAxis"}],"plot_height":350,"plot_width":350,"renderers":[{"id":"31653","type":"GlyphRenderer"}],"title":null,"toolbar":{"id":"31642","type":"Toolbar"},"toolbar_location":null,"x_range":{"id":"31617","type":"DataRange1d"},"x_scale":{"id":"31621","type":"LinearScale"},"y_range":{"id":"31619","type":"DataRange1d"},"y_scale":{"id":"31623","type":"LinearScale"}},"id":"31615","subtype":"Figure","type":"Plot"},{"attributes":{"source":{"id":"31614","type":"ColumnDataSource"}},"id":"31654","type":"CDSView"},{"attributes":{"children":[[{"id":"31615","subtype":"Figure","type":"Plot"},0,0],[{"id":"31655","subtype":"Figure","type":"Plot"},0,1]]},"id":"31717","type":"GridBox"},{"attributes":{"data_source":{"id":"31614","type":"ColumnDataSource"},"glyph":{"id":"31651","type":"Circle"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"31652","type":"Circle"},"selection_glyph":null,"view":{"id":"31654","type":"CDSView"}},"id":"31653","type":"GlyphRenderer"},{"attributes":{"formatter":{"id":"31710","type":"BasicTickFormatter"},"ticker":{"id":"31671","type":"BasicTicker"}},"id":"31670","type":"LinearAxis"},{"attributes":{},"id":"31697","type":"BasicTickFormatter"},{"attributes":{"ticker":{"id":"31666","type":"BasicTicker"}},"id":"31669","type":"Grid"},{"attributes":{},"id":"31699","type":"BasicTickFormatter"},{"attributes":{"below":[{"id":"31665","type":"LinearAxis"}],"center":[{"id":"31669","type":"Grid"},{"id":"31674","type":"Grid"}],"left":[{"id":"31670","type":"LinearAxis"}],"plot_height":350,"plot_width":350,"renderers":[{"id":"31693","type":"GlyphRenderer"}],"title":null,"toolbar":{"id":"31682","type":"Toolbar"},"toolbar_location":null,"x_range":{"id":"31657","type":"DataRange1d"},"x_scale":{"id":"31661","type":"LinearScale"},"y_range":{"id":"31659","type":"DataRange1d"},"y_scale":{"id":"31663","type":"LinearScale"}},"id":"31655","subtype":"Figure","type":"Plot"},{"attributes":{"callback":null},"id":"31617","type":"DataRange1d"},{"attributes":{"dimension":1,"ticker":{"id":"31671","type":"BasicTicker"}},"id":"31674","type":"Grid"},{"attributes":{},"id":"31663","type":"LinearScale"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"31700","type":"BoxAnnotation"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#1f77b4"},"line_alpha":{"value":0.1},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y1"}},"id":"31692","type":"Circle"},{"attributes":{"ticker":{"id":"31626","type":"BasicTicker"}},"id":"31629","type":"Grid"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"31701","type":"BoxAnnotation"},{"attributes":{},"id":"31661","type":"LinearScale"},{"attributes":{"callback":null},"id":"31619","type":"DataRange1d"},{"attributes":{"fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"xs_units":"screen","ys_units":"screen"},"id":"31702","type":"PolyAnnotation"},{"attributes":{"callback":null},"id":"31657","type":"DataRange1d"},{"attributes":{},"id":"31708","type":"BasicTickFormatter"},{"attributes":{},"id":"31636","type":"WheelZoomTool"},{"attributes":{},"id":"31703","type":"UnionRenderers"},{"attributes":{},"id":"31623","type":"LinearScale"},{"attributes":{"callback":null},"id":"31659","type":"DataRange1d"},{"attributes":{"dimension":1,"ticker":{"id":"31631","type":"BasicTicker"}},"id":"31634","type":"Grid"},{"attributes":{},"id":"31704","type":"Selection"},{"attributes":{},"id":"31710","type":"BasicTickFormatter"}],"root_ids":["31720"]},"title":"Bokeh Application","version":"1.3.0"}}';
                  var render_items = [{"docid":"83ebec1d-21f4-46cf-a994-bfe37d46e3af","roots":{"31720":"3af0a73d-c649-4eb1-a75d-b53420d6b763"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        embed_document(root);
                        clearInterval(timer);
                      }
                      attempts++;
                      if (attempts > 100) {
                        console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        clearInterval(timer);
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {} // ensure no trailing comma for IE
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();