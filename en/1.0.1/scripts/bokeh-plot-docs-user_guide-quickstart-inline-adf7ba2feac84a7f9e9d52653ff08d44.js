(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof (root._bokeh_onload_callbacks) === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) { callback() });
        }
        finally {
          delete root._bokeh_onload_callbacks
        }
        console.info("Bokeh: all callbacks have finished");
      }
    
      function load_libs(js_urls, callback) {
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.log("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.log("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = js_urls.length;
        for (var i = 0; i < js_urls.length; i++) {
          var url = js_urls[i];
          var s = document.createElement('script');
          s.src = url;
          s.async = false;
          s.onreadystatechange = s.onload = function() {
            root._bokeh_is_loading--;
            if (root._bokeh_is_loading === 0) {
              console.log("Bokeh: all BokehJS libraries loaded");
              run_callbacks()
            }
          };
          s.onerror = function() {
            console.warn("failed to load library " + url);
          };
          console.log("Bokeh: injecting script tag for BokehJS library: ", url);
          document.getElementsByTagName("head")[0].appendChild(s);
        }
      };var element = document.getElementById("d98103ef-787f-4766-97fe-87a73a9cd787");
      if (element == null) {
        console.log("Bokeh: ERROR: autoload.js configured with elementid 'd98103ef-787f-4766-97fe-87a73a9cd787' but no matching script tag was found. ")
        return false;
      }
    
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-1.0.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-1.0.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-1.0.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-1.0.1.min.js"];
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"4a844f86-a7d4-4181-ba55-d1df43163f33":{"roots":{"references":[{"attributes":{"plot":{"id":"36193","subtype":"Figure","type":"Plot"},"ticker":{"id":"36204","type":"BasicTicker"}},"id":"36207","type":"Grid"},{"attributes":{"below":[{"id":"36203","type":"LinearAxis"}],"left":[{"id":"36208","type":"LinearAxis"}],"plot_height":350,"plot_width":350,"renderers":[{"id":"36203","type":"LinearAxis"},{"id":"36207","type":"Grid"},{"id":"36208","type":"LinearAxis"},{"id":"36212","type":"Grid"},{"id":"36222","type":"BoxAnnotation"},{"id":"36227","type":"BoxAnnotation"},{"id":"36230","type":"PolyAnnotation"},{"id":"36237","type":"GlyphRenderer"}],"title":null,"toolbar":{"id":"36220","type":"Toolbar"},"toolbar_location":null,"x_range":{"id":"36195","type":"DataRange1d"},"x_scale":{"id":"36199","type":"LinearScale"},"y_range":{"id":"36197","type":"DataRange1d"},"y_scale":{"id":"36201","type":"LinearScale"}},"id":"36193","subtype":"Figure","type":"Plot"},{"attributes":{"formatter":{"id":"36243","type":"BasicTickFormatter"},"plot":{"id":"36147","subtype":"Figure","type":"Plot"},"ticker":{"id":"36163","type":"BasicTicker"}},"id":"36162","type":"LinearAxis"},{"attributes":{"plot":{"id":"36147","subtype":"Figure","type":"Plot"},"ticker":{"id":"36158","type":"BasicTicker"}},"id":"36161","type":"Grid"},{"attributes":{"callback":null,"data":{"x":{"__ndarray__":"","dtype":"float64","shape":[300]},"y0":{"__ndarray__":"","dtype":"float64","shape":[300]},"y1":{"__ndarray__":"","dtype":"float64","shape":[300]}},"selected":{"id":"36247","type":"Selection"},"selection_policy":{"id":"36248","type":"UnionRenderers"}},"id":"36146","type":"ColumnDataSource"},{"attributes":{"below":[{"id":"36157","type":"LinearAxis"}],"left":[{"id":"36162","type":"LinearAxis"}],"plot_height":350,"plot_width":350,"renderers":[{"id":"36157","type":"LinearAxis"},{"id":"36161","type":"Grid"},{"id":"36162","type":"LinearAxis"},{"id":"36166","type":"Grid"},{"id":"36176","type":"BoxAnnotation"},{"id":"36181","type":"BoxAnnotation"},{"id":"36184","type":"PolyAnnotation"},{"id":"36191","type":"GlyphRenderer"}],"title":null,"toolbar":{"id":"36174","type":"Toolbar"},"toolbar_location":null,"x_range":{"id":"36149","type":"DataRange1d"},"x_scale":{"id":"36153","type":"LinearScale"},"y_range":{"id":"36151","type":"DataRange1d"},"y_scale":{"id":"36155","type":"LinearScale"}},"id":"36147","subtype":"Figure","type":"Plot"},{"attributes":{},"id":"36209","type":"BasicTicker"},{"attributes":{"children":[{"id":"36261","type":"Row"}]},"id":"36262","type":"Column"},{"attributes":{},"id":"36217","type":"SaveTool"},{"attributes":{},"id":"36155","type":"LinearScale"},{"attributes":{"callback":null},"id":"36197","type":"DataRange1d"},{"attributes":{"source":{"id":"36146","type":"ColumnDataSource"}},"id":"36192","type":"CDSView"},{"attributes":{"source":{"id":"36146","type":"ColumnDataSource"}},"id":"36238","type":"CDSView"},{"attributes":{"callback":null,"overlay":{"id":"36184","type":"PolyAnnotation"}},"id":"36173","type":"LassoSelectTool"},{"attributes":{},"id":"36247","type":"Selection"},{"attributes":{"fill_color":{"value":"#1f77b4"},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y1"}},"id":"36235","type":"Circle"},{"attributes":{"formatter":{"id":"36241","type":"BasicTickFormatter"},"plot":{"id":"36147","subtype":"Figure","type":"Plot"},"ticker":{"id":"36158","type":"BasicTicker"}},"id":"36157","type":"LinearAxis"},{"attributes":{},"id":"36163","type":"BasicTicker"},{"attributes":{},"id":"36171","type":"SaveTool"},{"attributes":{},"id":"36201","type":"LinearScale"},{"attributes":{},"id":"36248","type":"UnionRenderers"},{"attributes":{"formatter":{"id":"36252","type":"BasicTickFormatter"},"plot":{"id":"36193","subtype":"Figure","type":"Plot"},"ticker":{"id":"36204","type":"BasicTicker"}},"id":"36203","type":"LinearAxis"},{"attributes":{"data_source":{"id":"36146","type":"ColumnDataSource"},"glyph":{"id":"36189","type":"Circle"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"36190","type":"Circle"},"selection_glyph":null,"view":{"id":"36192","type":"CDSView"}},"id":"36191","type":"GlyphRenderer"},{"attributes":{"callback":null,"overlay":{"id":"36230","type":"PolyAnnotation"}},"id":"36219","type":"LassoSelectTool"},{"attributes":{"children":[{"id":"36264","type":"ToolbarBox"},{"id":"36262","type":"Column"}]},"id":"36265","type":"Column"},{"attributes":{"callback":null},"id":"36149","type":"DataRange1d"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#1f77b4"},"line_alpha":{"value":0.1},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y0"}},"id":"36190","type":"Circle"},{"attributes":{"toolbar":{"id":"36263","type":"ProxyToolbar"},"toolbar_location":"above"},"id":"36264","type":"ToolbarBox"},{"attributes":{"callback":null},"id":"36151","type":"DataRange1d"},{"attributes":{"tools":[{"id":"36167","type":"PanTool"},{"id":"36168","type":"WheelZoomTool"},{"id":"36169","type":"BoxZoomTool"},{"id":"36170","type":"ResetTool"},{"id":"36171","type":"SaveTool"},{"id":"36172","type":"BoxSelectTool"},{"id":"36173","type":"LassoSelectTool"},{"id":"36213","type":"PanTool"},{"id":"36214","type":"WheelZoomTool"},{"id":"36215","type":"BoxZoomTool"},{"id":"36216","type":"ResetTool"},{"id":"36217","type":"SaveTool"},{"id":"36218","type":"BoxSelectTool"},{"id":"36219","type":"LassoSelectTool"}]},"id":"36263","type":"ProxyToolbar"},{"attributes":{"callback":null,"overlay":{"id":"36181","type":"BoxAnnotation"}},"id":"36172","type":"BoxSelectTool"},{"attributes":{},"id":"36170","type":"ResetTool"},{"attributes":{"formatter":{"id":"36254","type":"BasicTickFormatter"},"plot":{"id":"36193","subtype":"Figure","type":"Plot"},"ticker":{"id":"36209","type":"BasicTicker"}},"id":"36208","type":"LinearAxis"},{"attributes":{"fill_color":{"value":"#1f77b4"},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y0"}},"id":"36189","type":"Circle"},{"attributes":{},"id":"36167","type":"PanTool"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"36222","type":"BoxAnnotation"},{"attributes":{},"id":"36153","type":"LinearScale"},{"attributes":{},"id":"36199","type":"LinearScale"},{"attributes":{"data_source":{"id":"36146","type":"ColumnDataSource"},"glyph":{"id":"36235","type":"Circle"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"36236","type":"Circle"},"selection_glyph":null,"view":{"id":"36238","type":"CDSView"}},"id":"36237","type":"GlyphRenderer"},{"attributes":{"callback":null},"id":"36195","type":"DataRange1d"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#1f77b4"},"line_alpha":{"value":0.1},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y1"}},"id":"36236","type":"Circle"},{"attributes":{},"id":"36158","type":"BasicTicker"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"36181","type":"BoxAnnotation"},{"attributes":{},"id":"36213","type":"PanTool"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"36213","type":"PanTool"},{"id":"36214","type":"WheelZoomTool"},{"id":"36215","type":"BoxZoomTool"},{"id":"36216","type":"ResetTool"},{"id":"36217","type":"SaveTool"},{"id":"36218","type":"BoxSelectTool"},{"id":"36219","type":"LassoSelectTool"}]},"id":"36220","type":"Toolbar"},{"attributes":{"children":[{"id":"36147","subtype":"Figure","type":"Plot"},{"id":"36193","subtype":"Figure","type":"Plot"}]},"id":"36261","type":"Row"},{"attributes":{},"id":"36204","type":"BasicTicker"},{"attributes":{},"id":"36243","type":"BasicTickFormatter"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"36176","type":"BoxAnnotation"},{"attributes":{"dimension":1,"plot":{"id":"36193","subtype":"Figure","type":"Plot"},"ticker":{"id":"36209","type":"BasicTicker"}},"id":"36212","type":"Grid"},{"attributes":{},"id":"36214","type":"WheelZoomTool"},{"attributes":{"overlay":{"id":"36176","type":"BoxAnnotation"}},"id":"36169","type":"BoxZoomTool"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"36167","type":"PanTool"},{"id":"36168","type":"WheelZoomTool"},{"id":"36169","type":"BoxZoomTool"},{"id":"36170","type":"ResetTool"},{"id":"36171","type":"SaveTool"},{"id":"36172","type":"BoxSelectTool"},{"id":"36173","type":"LassoSelectTool"}]},"id":"36174","type":"Toolbar"},{"attributes":{},"id":"36241","type":"BasicTickFormatter"},{"attributes":{},"id":"36252","type":"BasicTickFormatter"},{"attributes":{},"id":"36216","type":"ResetTool"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"36227","type":"BoxAnnotation"},{"attributes":{"fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"xs_units":"screen","ys_units":"screen"},"id":"36184","type":"PolyAnnotation"},{"attributes":{},"id":"36254","type":"BasicTickFormatter"},{"attributes":{"callback":null,"overlay":{"id":"36227","type":"BoxAnnotation"}},"id":"36218","type":"BoxSelectTool"},{"attributes":{"dimension":1,"plot":{"id":"36147","subtype":"Figure","type":"Plot"},"ticker":{"id":"36163","type":"BasicTicker"}},"id":"36166","type":"Grid"},{"attributes":{},"id":"36168","type":"WheelZoomTool"},{"attributes":{"overlay":{"id":"36222","type":"BoxAnnotation"}},"id":"36215","type":"BoxZoomTool"},{"attributes":{"fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"xs_units":"screen","ys_units":"screen"},"id":"36230","type":"PolyAnnotation"}],"root_ids":["36265"]},"title":"Bokeh Application","version":"1.0.1"}}';
                  var render_items = [{"docid":"4a844f86-a7d4-4181-ba55-d1df43163f33","roots":{"36265":"d98103ef-787f-4766-97fe-87a73a9cd787"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        embed_document(root);
                        clearInterval(timer);
                      }
                      attempts++;
                      if (attempts > 100) {
                        console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        clearInterval(timer);
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
          console.log("Bokeh: injecting CSS: https://cdn.bokeh.org/bokeh/release/bokeh-1.0.1.min.css");
          Bokeh.embed.inject_css("https://cdn.bokeh.org/bokeh/release/bokeh-1.0.1.min.css");
          console.log("Bokeh: injecting CSS: https://cdn.bokeh.org/bokeh/release/bokeh-widgets-1.0.1.min.css");
          Bokeh.embed.inject_css("https://cdn.bokeh.org/bokeh/release/bokeh-widgets-1.0.1.min.css");
          console.log("Bokeh: injecting CSS: https://cdn.bokeh.org/bokeh/release/bokeh-tables-1.0.1.min.css");
          Bokeh.embed.inject_css("https://cdn.bokeh.org/bokeh/release/bokeh-tables-1.0.1.min.css");
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.log("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(js_urls, function() {
          console.log("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();