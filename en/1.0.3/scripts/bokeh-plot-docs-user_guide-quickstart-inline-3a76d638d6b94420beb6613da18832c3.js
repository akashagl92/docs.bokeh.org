(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof (root._bokeh_onload_callbacks) === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) { callback() });
        }
        finally {
          delete root._bokeh_onload_callbacks
        }
        console.info("Bokeh: all callbacks have finished");
      }
    
      function load_libs(js_urls, callback) {
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.log("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.log("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = js_urls.length;
        for (var i = 0; i < js_urls.length; i++) {
          var url = js_urls[i];
          var s = document.createElement('script');
          s.src = url;
          s.async = false;
          s.onreadystatechange = s.onload = function() {
            root._bokeh_is_loading--;
            if (root._bokeh_is_loading === 0) {
              console.log("Bokeh: all BokehJS libraries loaded");
              run_callbacks()
            }
          };
          s.onerror = function() {
            console.warn("failed to load library " + url);
          };
          console.log("Bokeh: injecting script tag for BokehJS library: ", url);
          document.getElementsByTagName("head")[0].appendChild(s);
        }
      };var element = document.getElementById("df6b9513-a1d1-427b-9162-35df4a701935");
      if (element == null) {
        console.log("Bokeh: ERROR: autoload.js configured with elementid 'df6b9513-a1d1-427b-9162-35df4a701935' but no matching script tag was found. ")
        return false;
      }
    
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-1.0.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-1.0.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-1.0.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-1.0.3.min.js"];
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"cb39451f-e983-49d4-8651-6c35cffb6cee":{"roots":{"references":[{"attributes":{},"id":"36165","type":"LinearScale"},{"attributes":{},"id":"36168","type":"BasicTicker"},{"attributes":{},"id":"36180","type":"ResetTool"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"36237","type":"BoxAnnotation"},{"attributes":{"callback":null},"id":"36205","type":"DataRange1d"},{"attributes":{"callback":null,"data":{"x":{"__ndarray__":"","dtype":"float64","shape":[300]},"y0":{"__ndarray__":"","dtype":"float64","shape":[300]},"y1":{"__ndarray__":"","dtype":"float64","shape":[300]}},"selected":{"id":"36257","type":"Selection"},"selection_policy":{"id":"36258","type":"UnionRenderers"}},"id":"36156","type":"ColumnDataSource"},{"attributes":{"fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"xs_units":"screen","ys_units":"screen"},"id":"36240","type":"PolyAnnotation"},{"attributes":{"tools":[{"id":"36177","type":"PanTool"},{"id":"36178","type":"WheelZoomTool"},{"id":"36179","type":"BoxZoomTool"},{"id":"36180","type":"ResetTool"},{"id":"36181","type":"SaveTool"},{"id":"36182","type":"BoxSelectTool"},{"id":"36183","type":"LassoSelectTool"},{"id":"36223","type":"PanTool"},{"id":"36224","type":"WheelZoomTool"},{"id":"36225","type":"BoxZoomTool"},{"id":"36226","type":"ResetTool"},{"id":"36227","type":"SaveTool"},{"id":"36228","type":"BoxSelectTool"},{"id":"36229","type":"LassoSelectTool"}]},"id":"36273","type":"ProxyToolbar"},{"attributes":{"fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"xs_units":"screen","ys_units":"screen"},"id":"36194","type":"PolyAnnotation"},{"attributes":{"callback":null},"id":"36159","type":"DataRange1d"},{"attributes":{"children":[{"id":"36274","type":"ToolbarBox"},{"id":"36272","type":"Column"}]},"id":"36275","type":"Column"},{"attributes":{"data_source":{"id":"36156","type":"ColumnDataSource"},"glyph":{"id":"36245","type":"Circle"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"36246","type":"Circle"},"selection_glyph":null,"view":{"id":"36248","type":"CDSView"}},"id":"36247","type":"GlyphRenderer"},{"attributes":{},"id":"36263","type":"BasicTickFormatter"},{"attributes":{"overlay":{"id":"36232","type":"BoxAnnotation"}},"id":"36225","type":"BoxZoomTool"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"36186","type":"BoxAnnotation"},{"attributes":{"source":{"id":"36156","type":"ColumnDataSource"}},"id":"36202","type":"CDSView"},{"attributes":{"formatter":{"id":"36250","type":"BasicTickFormatter"},"plot":{"id":"36157","subtype":"Figure","type":"Plot"},"ticker":{"id":"36173","type":"BasicTicker"}},"id":"36172","type":"LinearAxis"},{"attributes":{"below":[{"id":"36213","type":"LinearAxis"}],"left":[{"id":"36218","type":"LinearAxis"}],"plot_height":350,"plot_width":350,"renderers":[{"id":"36213","type":"LinearAxis"},{"id":"36217","type":"Grid"},{"id":"36218","type":"LinearAxis"},{"id":"36222","type":"Grid"},{"id":"36232","type":"BoxAnnotation"},{"id":"36237","type":"BoxAnnotation"},{"id":"36240","type":"PolyAnnotation"},{"id":"36247","type":"GlyphRenderer"}],"title":null,"toolbar":{"id":"36230","type":"Toolbar"},"toolbar_location":null,"x_range":{"id":"36205","type":"DataRange1d"},"x_scale":{"id":"36209","type":"LinearScale"},"y_range":{"id":"36207","type":"DataRange1d"},"y_scale":{"id":"36211","type":"LinearScale"}},"id":"36203","subtype":"Figure","type":"Plot"},{"attributes":{"plot":{"id":"36157","subtype":"Figure","type":"Plot"},"ticker":{"id":"36168","type":"BasicTicker"}},"id":"36171","type":"Grid"},{"attributes":{"callback":null,"overlay":{"id":"36237","type":"BoxAnnotation"}},"id":"36228","type":"BoxSelectTool"},{"attributes":{"fill_color":{"value":"#1f77b4"},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y1"}},"id":"36245","type":"Circle"},{"attributes":{"formatter":{"id":"36261","type":"BasicTickFormatter"},"plot":{"id":"36203","subtype":"Figure","type":"Plot"},"ticker":{"id":"36219","type":"BasicTicker"}},"id":"36218","type":"LinearAxis"},{"attributes":{},"id":"36261","type":"BasicTickFormatter"},{"attributes":{"plot":{"id":"36203","subtype":"Figure","type":"Plot"},"ticker":{"id":"36214","type":"BasicTicker"}},"id":"36217","type":"Grid"},{"attributes":{"callback":null,"overlay":{"id":"36191","type":"BoxAnnotation"}},"id":"36182","type":"BoxSelectTool"},{"attributes":{},"id":"36209","type":"LinearScale"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"36177","type":"PanTool"},{"id":"36178","type":"WheelZoomTool"},{"id":"36179","type":"BoxZoomTool"},{"id":"36180","type":"ResetTool"},{"id":"36181","type":"SaveTool"},{"id":"36182","type":"BoxSelectTool"},{"id":"36183","type":"LassoSelectTool"}]},"id":"36184","type":"Toolbar"},{"attributes":{},"id":"36252","type":"BasicTickFormatter"},{"attributes":{},"id":"36223","type":"PanTool"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#1f77b4"},"line_alpha":{"value":0.1},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y0"}},"id":"36200","type":"Circle"},{"attributes":{"dimension":1,"plot":{"id":"36203","subtype":"Figure","type":"Plot"},"ticker":{"id":"36219","type":"BasicTicker"}},"id":"36222","type":"Grid"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"36223","type":"PanTool"},{"id":"36224","type":"WheelZoomTool"},{"id":"36225","type":"BoxZoomTool"},{"id":"36226","type":"ResetTool"},{"id":"36227","type":"SaveTool"},{"id":"36228","type":"BoxSelectTool"},{"id":"36229","type":"LassoSelectTool"}]},"id":"36230","type":"Toolbar"},{"attributes":{"children":[{"id":"36157","subtype":"Figure","type":"Plot"},{"id":"36203","subtype":"Figure","type":"Plot"}]},"id":"36271","type":"Row"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"36232","type":"BoxAnnotation"},{"attributes":{},"id":"36219","type":"BasicTicker"},{"attributes":{"dimension":1,"plot":{"id":"36157","subtype":"Figure","type":"Plot"},"ticker":{"id":"36173","type":"BasicTicker"}},"id":"36176","type":"Grid"},{"attributes":{},"id":"36257","type":"Selection"},{"attributes":{"callback":null},"id":"36207","type":"DataRange1d"},{"attributes":{},"id":"36224","type":"WheelZoomTool"},{"attributes":{},"id":"36173","type":"BasicTicker"},{"attributes":{},"id":"36226","type":"ResetTool"},{"attributes":{"children":[{"id":"36271","type":"Row"}]},"id":"36272","type":"Column"},{"attributes":{"data_source":{"id":"36156","type":"ColumnDataSource"},"glyph":{"id":"36199","type":"Circle"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"36200","type":"Circle"},"selection_glyph":null,"view":{"id":"36202","type":"CDSView"}},"id":"36201","type":"GlyphRenderer"},{"attributes":{},"id":"36163","type":"LinearScale"},{"attributes":{},"id":"36178","type":"WheelZoomTool"},{"attributes":{},"id":"36227","type":"SaveTool"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"36191","type":"BoxAnnotation"},{"attributes":{},"id":"36214","type":"BasicTicker"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#1f77b4"},"line_alpha":{"value":0.1},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y1"}},"id":"36246","type":"Circle"},{"attributes":{"callback":null},"id":"36161","type":"DataRange1d"},{"attributes":{},"id":"36181","type":"SaveTool"},{"attributes":{"callback":null,"overlay":{"id":"36240","type":"PolyAnnotation"}},"id":"36229","type":"LassoSelectTool"},{"attributes":{"below":[{"id":"36167","type":"LinearAxis"}],"left":[{"id":"36172","type":"LinearAxis"}],"plot_height":350,"plot_width":350,"renderers":[{"id":"36167","type":"LinearAxis"},{"id":"36171","type":"Grid"},{"id":"36172","type":"LinearAxis"},{"id":"36176","type":"Grid"},{"id":"36186","type":"BoxAnnotation"},{"id":"36191","type":"BoxAnnotation"},{"id":"36194","type":"PolyAnnotation"},{"id":"36201","type":"GlyphRenderer"}],"title":null,"toolbar":{"id":"36184","type":"Toolbar"},"toolbar_location":null,"x_range":{"id":"36159","type":"DataRange1d"},"x_scale":{"id":"36163","type":"LinearScale"},"y_range":{"id":"36161","type":"DataRange1d"},"y_scale":{"id":"36165","type":"LinearScale"}},"id":"36157","subtype":"Figure","type":"Plot"},{"attributes":{"formatter":{"id":"36252","type":"BasicTickFormatter"},"plot":{"id":"36157","subtype":"Figure","type":"Plot"},"ticker":{"id":"36168","type":"BasicTicker"}},"id":"36167","type":"LinearAxis"},{"attributes":{"formatter":{"id":"36263","type":"BasicTickFormatter"},"plot":{"id":"36203","subtype":"Figure","type":"Plot"},"ticker":{"id":"36214","type":"BasicTicker"}},"id":"36213","type":"LinearAxis"},{"attributes":{"fill_color":{"value":"#1f77b4"},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y0"}},"id":"36199","type":"Circle"},{"attributes":{"overlay":{"id":"36186","type":"BoxAnnotation"}},"id":"36179","type":"BoxZoomTool"},{"attributes":{"callback":null,"overlay":{"id":"36194","type":"PolyAnnotation"}},"id":"36183","type":"LassoSelectTool"},{"attributes":{},"id":"36211","type":"LinearScale"},{"attributes":{},"id":"36250","type":"BasicTickFormatter"},{"attributes":{"toolbar":{"id":"36273","type":"ProxyToolbar"},"toolbar_location":"above"},"id":"36274","type":"ToolbarBox"},{"attributes":{},"id":"36177","type":"PanTool"},{"attributes":{"source":{"id":"36156","type":"ColumnDataSource"}},"id":"36248","type":"CDSView"},{"attributes":{},"id":"36258","type":"UnionRenderers"}],"root_ids":["36275"]},"title":"Bokeh Application","version":"1.0.3"}}';
                  var render_items = [{"docid":"cb39451f-e983-49d4-8651-6c35cffb6cee","roots":{"36275":"df6b9513-a1d1-427b-9162-35df4a701935"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        embed_document(root);
                        clearInterval(timer);
                      }
                      attempts++;
                      if (attempts > 100) {
                        console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        clearInterval(timer);
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
          console.log("Bokeh: injecting CSS: https://cdn.bokeh.org/bokeh/release/bokeh-1.0.3.min.css");
          Bokeh.embed.inject_css("https://cdn.bokeh.org/bokeh/release/bokeh-1.0.3.min.css");
          console.log("Bokeh: injecting CSS: https://cdn.bokeh.org/bokeh/release/bokeh-widgets-1.0.3.min.css");
          Bokeh.embed.inject_css("https://cdn.bokeh.org/bokeh/release/bokeh-widgets-1.0.3.min.css");
          console.log("Bokeh: injecting CSS: https://cdn.bokeh.org/bokeh/release/bokeh-tables-1.0.3.min.css");
          Bokeh.embed.inject_css("https://cdn.bokeh.org/bokeh/release/bokeh-tables-1.0.3.min.css");
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.log("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(js_urls, function() {
          console.log("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();