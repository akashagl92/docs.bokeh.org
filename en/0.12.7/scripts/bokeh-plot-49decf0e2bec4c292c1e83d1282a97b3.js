(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof (root._bokeh_onload_callbacks) === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
    
      
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) { callback() });
        }
        finally {
          delete root._bokeh_onload_callbacks
        }
        console.info("Bokeh: all callbacks have finished");
      }
    
      function load_libs(js_urls, callback) {
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.log("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.log("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = js_urls.length;
        for (var i = 0; i < js_urls.length; i++) {
          var url = js_urls[i];
          var s = document.createElement('script');
          s.src = url;
          s.async = false;
          s.onreadystatechange = s.onload = function() {
            root._bokeh_is_loading--;
            if (root._bokeh_is_loading === 0) {
              console.log("Bokeh: all BokehJS libraries loaded");
              run_callbacks()
            }
          };
          s.onerror = function() {
            console.warn("failed to load library " + url);
          };
          console.log("Bokeh: injecting script tag for BokehJS library: ", url);
          document.getElementsByTagName("head")[0].appendChild(s);
        }
      };var element = document.getElementById("1a3808cc-eb8c-4143-aff2-7eaf2d5b3b08");
      if (element == null) {
        console.log("Bokeh: ERROR: autoload.js configured with elementid '1a3808cc-eb8c-4143-aff2-7eaf2d5b3b08' but no matching script tag was found. ")
        return false;
      }
    
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-0.12.7.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-0.12.7.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-0.12.7.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-0.12.7.min.js"];
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                var docs_json = {"a76234c4-0fb7-461c-9f30-6679fd83bd43":{"roots":{"references":[{"attributes":{"plot":null,"text":""},"id":"52fda2c2-5f4b-4ccb-b455-95ed12953c6a","type":"Title"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_scroll":"auto","active_tap":"auto","tools":[{"id":"e61a18e7-0f94-40b2-89ab-e3350d6026bd","type":"PanTool"},{"id":"474a2507-c80d-401c-bc89-b8c366750a90","type":"WheelZoomTool"},{"id":"b2d021eb-837f-4c97-ab7e-4e891728a019","type":"BoxZoomTool"},{"id":"a772ac80-0ca5-4ac9-8a94-845fc501c1a8","type":"SaveTool"},{"id":"c5e37e62-d67c-45bd-be98-13e183d46476","type":"ResetTool"},{"id":"58510061-c229-4cd5-aa73-4b83f0d09dd7","type":"HelpTool"}]},"id":"62277850-61a9-4583-9670-727699d4c666","type":"Toolbar"},{"attributes":{"callback":{"id":"8faef211-4ca8-472a-a781-35bf57eb91b0","type":"CustomJS"},"end":6.4,"start":0,"step":0.1,"title":"Phase","value":0},"id":"0b866539-3619-4e9d-ad8d-6617177adfa8","type":"Slider"},{"attributes":{"formatter":{"id":"1cf09e1c-3aab-4436-96ec-44eaed3fbaa7","type":"BasicTickFormatter"},"plot":{"id":"84463db1-4886-4c94-aad8-272c3842daec","subtype":"Figure","type":"Plot"},"ticker":{"id":"dcdee71b-37dd-4aa7-b71e-5f7a3e07403f","type":"BasicTicker"}},"id":"bd436bb1-cf05-4b94-8d86-59cf4e07fbf4","type":"LinearAxis"},{"attributes":{"callback":null},"id":"441475d3-18c1-4add-baef-4165ae9af8b6","type":"DataRange1d"},{"attributes":{"children":[{"id":"22383adb-e5ac-423e-bb29-06621d725708","type":"Slider"},{"id":"9c13c6bb-dc73-4dc5-8450-5dfe80ef80a2","type":"Slider"},{"id":"0b866539-3619-4e9d-ad8d-6617177adfa8","type":"Slider"},{"id":"1b4fef51-0bb0-4719-b298-cbbdffb7a15b","type":"Slider"}]},"id":"caec8e6a-d426-41f6-8137-a4fc508d069a","type":"WidgetBox"},{"attributes":{},"id":"1cf09e1c-3aab-4436-96ec-44eaed3fbaa7","type":"BasicTickFormatter"},{"attributes":{"callback":null,"column_names":["x","y"],"data":{"x":{"__ndarray__":"","dtype":"float64","shape":[500]},"y":{"__ndarray__":"","dtype":"float64","shape":[500]}}},"id":"53dc8f6f-9ba1-47c4-849b-a5cec20e5f52","type":"ColumnDataSource"},{"attributes":{},"id":"c5e37e62-d67c-45bd-be98-13e183d46476","type":"ResetTool"},{"attributes":{"overlay":{"id":"dc23b3a7-b129-435f-abb5-b65e9b6b5ba8","type":"BoxAnnotation"}},"id":"b2d021eb-837f-4c97-ab7e-4e891728a019","type":"BoxZoomTool"},{"attributes":{"line_alpha":{"value":0.6},"line_color":{"value":"#1f77b4"},"line_width":{"value":3},"x":{"field":"x"},"y":{"field":"y"}},"id":"127030e5-12b1-4ba7-81de-d87ee0a608d7","type":"Line"},{"attributes":{"formatter":{"id":"34b9ff66-d1ba-4455-bac8-c6803f311b32","type":"BasicTickFormatter"},"plot":{"id":"84463db1-4886-4c94-aad8-272c3842daec","subtype":"Figure","type":"Plot"},"ticker":{"id":"b45422f8-b9c2-4334-968f-e6e878efc1d3","type":"BasicTicker"}},"id":"47f5c712-e76f-46d8-b5c2-f50ab4e499e3","type":"LinearAxis"},{"attributes":{"callback":{"id":"8faef211-4ca8-472a-a781-35bf57eb91b0","type":"CustomJS"},"end":10,"start":0.1,"step":0.1,"title":"Amplitude","value":1},"id":"22383adb-e5ac-423e-bb29-06621d725708","type":"Slider"},{"attributes":{"below":[{"id":"47f5c712-e76f-46d8-b5c2-f50ab4e499e3","type":"LinearAxis"}],"left":[{"id":"bd436bb1-cf05-4b94-8d86-59cf4e07fbf4","type":"LinearAxis"}],"plot_height":400,"plot_width":400,"renderers":[{"id":"47f5c712-e76f-46d8-b5c2-f50ab4e499e3","type":"LinearAxis"},{"id":"8ece2f90-47a8-4be4-b902-0643533cc2f0","type":"Grid"},{"id":"bd436bb1-cf05-4b94-8d86-59cf4e07fbf4","type":"LinearAxis"},{"id":"70c761f9-3dcd-4099-8a6c-aa282d0f5852","type":"Grid"},{"id":"dc23b3a7-b129-435f-abb5-b65e9b6b5ba8","type":"BoxAnnotation"},{"id":"e470bc9d-9188-4333-9296-e2a8f20e0e39","type":"GlyphRenderer"}],"title":{"id":"52fda2c2-5f4b-4ccb-b455-95ed12953c6a","type":"Title"},"toolbar":{"id":"62277850-61a9-4583-9670-727699d4c666","type":"Toolbar"},"x_range":{"id":"441475d3-18c1-4add-baef-4165ae9af8b6","type":"DataRange1d"},"x_scale":{"id":"7cac39f4-0b1e-4efa-a603-479c4e523725","type":"LinearScale"},"y_range":{"id":"ca96922a-6dd2-489e-b6bc-0ffe1552ef23","type":"Range1d"},"y_scale":{"id":"67af8baa-afca-43ea-8afd-89c2300db32e","type":"LinearScale"}},"id":"84463db1-4886-4c94-aad8-272c3842daec","subtype":"Figure","type":"Plot"},{"attributes":{},"id":"dcdee71b-37dd-4aa7-b71e-5f7a3e07403f","type":"BasicTicker"},{"attributes":{"dimension":1,"plot":{"id":"84463db1-4886-4c94-aad8-272c3842daec","subtype":"Figure","type":"Plot"},"ticker":{"id":"dcdee71b-37dd-4aa7-b71e-5f7a3e07403f","type":"BasicTicker"}},"id":"70c761f9-3dcd-4099-8a6c-aa282d0f5852","type":"Grid"},{"attributes":{"data_source":{"id":"53dc8f6f-9ba1-47c4-849b-a5cec20e5f52","type":"ColumnDataSource"},"glyph":{"id":"127030e5-12b1-4ba7-81de-d87ee0a608d7","type":"Line"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"1a2b0a1a-ae2c-4079-a359-4038fd88bee7","type":"Line"},"selection_glyph":null,"view":{"id":"cefd57e3-cb53-4e21-941d-3758edf821b0","type":"CDSView"}},"id":"e470bc9d-9188-4333-9296-e2a8f20e0e39","type":"GlyphRenderer"},{"attributes":{"children":[{"id":"84463db1-4886-4c94-aad8-272c3842daec","subtype":"Figure","type":"Plot"},{"id":"caec8e6a-d426-41f6-8137-a4fc508d069a","type":"WidgetBox"}]},"id":"acef71a0-875c-46f0-8414-5c249747d658","type":"Row"},{"attributes":{},"id":"7cac39f4-0b1e-4efa-a603-479c4e523725","type":"LinearScale"},{"attributes":{"line_alpha":{"value":0.1},"line_color":{"value":"#1f77b4"},"line_width":{"value":3},"x":{"field":"x"},"y":{"field":"y"}},"id":"1a2b0a1a-ae2c-4079-a359-4038fd88bee7","type":"Line"},{"attributes":{"args":{"amp":{"id":"22383adb-e5ac-423e-bb29-06621d725708","type":"Slider"},"freq":{"id":"9c13c6bb-dc73-4dc5-8450-5dfe80ef80a2","type":"Slider"},"offset":{"id":"1b4fef51-0bb0-4719-b298-cbbdffb7a15b","type":"Slider"},"phase":{"id":"0b866539-3619-4e9d-ad8d-6617177adfa8","type":"Slider"},"source":{"id":"53dc8f6f-9ba1-47c4-849b-a5cec20e5f52","type":"ColumnDataSource"}},"code":"\n    var data = source.data;\n    var A = amp.value;\n    var k = freq.value;\n    var phi = phase.value;\n    var B = offset.value;\n    x = data['x']\n    y = data['y']\n    for (i = 0; i < x.length; i++) {\n        y[i] = B + A*Math.sin(k*x[i]+phi);\n    }\n    source.change.emit();\n"},"id":"8faef211-4ca8-472a-a781-35bf57eb91b0","type":"CustomJS"},{"attributes":{},"id":"a772ac80-0ca5-4ac9-8a94-845fc501c1a8","type":"SaveTool"},{"attributes":{"callback":null,"end":10,"start":-10},"id":"ca96922a-6dd2-489e-b6bc-0ffe1552ef23","type":"Range1d"},{"attributes":{"callback":{"id":"8faef211-4ca8-472a-a781-35bf57eb91b0","type":"CustomJS"},"end":10,"start":0.1,"step":0.1,"title":"Frequency","value":1},"id":"9c13c6bb-dc73-4dc5-8450-5dfe80ef80a2","type":"Slider"},{"attributes":{},"id":"474a2507-c80d-401c-bc89-b8c366750a90","type":"WheelZoomTool"},{"attributes":{"plot":{"id":"84463db1-4886-4c94-aad8-272c3842daec","subtype":"Figure","type":"Plot"},"ticker":{"id":"b45422f8-b9c2-4334-968f-e6e878efc1d3","type":"BasicTicker"}},"id":"8ece2f90-47a8-4be4-b902-0643533cc2f0","type":"Grid"},{"attributes":{"callback":{"id":"8faef211-4ca8-472a-a781-35bf57eb91b0","type":"CustomJS"},"end":5,"start":-5,"step":0.1,"title":"Offset","value":0},"id":"1b4fef51-0bb0-4719-b298-cbbdffb7a15b","type":"Slider"},{"attributes":{},"id":"67af8baa-afca-43ea-8afd-89c2300db32e","type":"LinearScale"},{"attributes":{"source":{"id":"53dc8f6f-9ba1-47c4-849b-a5cec20e5f52","type":"ColumnDataSource"}},"id":"cefd57e3-cb53-4e21-941d-3758edf821b0","type":"CDSView"},{"attributes":{},"id":"34b9ff66-d1ba-4455-bac8-c6803f311b32","type":"BasicTickFormatter"},{"attributes":{},"id":"e61a18e7-0f94-40b2-89ab-e3350d6026bd","type":"PanTool"},{"attributes":{},"id":"b45422f8-b9c2-4334-968f-e6e878efc1d3","type":"BasicTicker"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"plot":null,"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"dc23b3a7-b129-435f-abb5-b65e9b6b5ba8","type":"BoxAnnotation"},{"attributes":{},"id":"58510061-c229-4cd5-aa73-4b83f0d09dd7","type":"HelpTool"}],"root_ids":["acef71a0-875c-46f0-8414-5c249747d658"]},"title":"Bokeh Application","version":"0.12.7"}};
                var render_items = [{"docid":"a76234c4-0fb7-461c-9f30-6679fd83bd43","elementid":"1a3808cc-eb8c-4143-aff2-7eaf2d5b3b08","modelid":"acef71a0-875c-46f0-8414-5c249747d658"}];
                
                Bokeh.embed.embed_items(docs_json, render_items);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
          console.log("Bokeh: injecting CSS: https://cdn.bokeh.org/bokeh/release/bokeh-0.12.7.min.css");
          Bokeh.embed.inject_css("https://cdn.bokeh.org/bokeh/release/bokeh-0.12.7.min.css");
          console.log("Bokeh: injecting CSS: https://cdn.bokeh.org/bokeh/release/bokeh-widgets-0.12.7.min.css");
          Bokeh.embed.inject_css("https://cdn.bokeh.org/bokeh/release/bokeh-widgets-0.12.7.min.css");
          console.log("Bokeh: injecting CSS: https://cdn.bokeh.org/bokeh/release/bokeh-tables-0.12.7.min.css");
          Bokeh.embed.inject_css("https://cdn.bokeh.org/bokeh/release/bokeh-tables-0.12.7.min.css");
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.log("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(js_urls, function() {
          console.log("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();
