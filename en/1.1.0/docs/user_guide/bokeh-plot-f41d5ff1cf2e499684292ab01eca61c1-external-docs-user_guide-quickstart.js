(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error() {
          console.error("failed to load " + url);
        }
    
        for (var i = 0; i < css_urls.length; i++) {
          var url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error;
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        for (var i = 0; i < js_urls.length; i++) {
          var url = js_urls[i];
          var element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error;
          element.async = false;
          element.src = url;
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };var element = document.getElementById("b5dc6748-4caa-432b-a1e8-0ec2b82d13e8");
      if (element == null) {
        console.error("Bokeh: ERROR: autoload.js configured with elementid 'b5dc6748-4caa-432b-a1e8-0ec2b82d13e8' but no matching script tag was found. ")
        return false;
      }
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-1.1.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-1.1.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-1.1.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-1.1.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-api-1.1.0.min.js"];
      var css_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-1.1.0.min.css", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-1.1.0.min.css", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-1.1.0.min.css"];
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"a31ae450-3732-4a22-ae8c-3a09e09111a4":{"roots":{"references":[{"attributes":{"fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"xs_units":"screen","ys_units":"screen"},"id":"30563","type":"PolyAnnotation"},{"attributes":{},"id":"30474","type":"BasicTicker"},{"attributes":{"dimension":1,"ticker":{"id":"30479","type":"BasicTicker"}},"id":"30482","type":"Grid"},{"attributes":{"callback":null,"overlay":{"id":"30562","type":"BoxAnnotation"}},"id":"30528","type":"BoxSelectTool"},{"attributes":{"tools":[{"id":"30483","type":"PanTool"},{"id":"30484","type":"WheelZoomTool"},{"id":"30485","type":"BoxZoomTool"},{"id":"30486","type":"ResetTool"},{"id":"30487","type":"SaveTool"},{"id":"30488","type":"BoxSelectTool"},{"id":"30489","type":"LassoSelectTool"},{"id":"30523","type":"PanTool"},{"id":"30524","type":"WheelZoomTool"},{"id":"30525","type":"BoxZoomTool"},{"id":"30526","type":"ResetTool"},{"id":"30527","type":"SaveTool"},{"id":"30528","type":"BoxSelectTool"},{"id":"30529","type":"LassoSelectTool"}]},"id":"30566","type":"ProxyToolbar"},{"attributes":{"source":{"id":"30462","type":"ColumnDataSource"}},"id":"30502","type":"CDSView"},{"attributes":{"callback":null,"overlay":{"id":"30563","type":"PolyAnnotation"}},"id":"30529","type":"LassoSelectTool"},{"attributes":{"toolbar":{"id":"30566","type":"ProxyToolbar"},"toolbar_location":"above"},"id":"30567","type":"ToolbarBox"},{"attributes":{"callback":null,"overlay":{"id":"30551","type":"BoxAnnotation"}},"id":"30488","type":"BoxSelectTool"},{"attributes":{"below":[{"id":"30513","type":"LinearAxis"}],"center":[{"id":"30517","type":"Grid"},{"id":"30522","type":"Grid"}],"left":[{"id":"30518","type":"LinearAxis"}],"plot_height":350,"plot_width":350,"renderers":[{"id":"30541","type":"GlyphRenderer"}],"title":null,"toolbar":{"id":"30530","type":"Toolbar"},"toolbar_location":null,"x_range":{"id":"30505","type":"DataRange1d"},"x_scale":{"id":"30509","type":"LinearScale"},"y_range":{"id":"30507","type":"DataRange1d"},"y_scale":{"id":"30511","type":"LinearScale"}},"id":"30503","subtype":"Figure","type":"Plot"},{"attributes":{"fill_color":{"value":"#1f77b4"},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y0"}},"id":"30499","type":"Circle"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"30523","type":"PanTool"},{"id":"30524","type":"WheelZoomTool"},{"id":"30525","type":"BoxZoomTool"},{"id":"30526","type":"ResetTool"},{"id":"30527","type":"SaveTool"},{"id":"30528","type":"BoxSelectTool"},{"id":"30529","type":"LassoSelectTool"}]},"id":"30530","type":"Toolbar"},{"attributes":{"children":[{"id":"30567","type":"ToolbarBox"},{"id":"30565","type":"GridBox"}]},"id":"30568","type":"Column"},{"attributes":{},"id":"30479","type":"BasicTicker"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#1f77b4"},"line_alpha":{"value":0.1},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y0"}},"id":"30500","type":"Circle"},{"attributes":{},"id":"30483","type":"PanTool"},{"attributes":{"data_source":{"id":"30462","type":"ColumnDataSource"},"glyph":{"id":"30499","type":"Circle"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"30500","type":"Circle"},"selection_glyph":null,"view":{"id":"30502","type":"CDSView"}},"id":"30501","type":"GlyphRenderer"},{"attributes":{"active_drag":"auto","active_inspect":"auto","active_multi":null,"active_scroll":"auto","active_tap":"auto","tools":[{"id":"30483","type":"PanTool"},{"id":"30484","type":"WheelZoomTool"},{"id":"30485","type":"BoxZoomTool"},{"id":"30486","type":"ResetTool"},{"id":"30487","type":"SaveTool"},{"id":"30488","type":"BoxSelectTool"},{"id":"30489","type":"LassoSelectTool"}]},"id":"30490","type":"Toolbar"},{"attributes":{"data_source":{"id":"30462","type":"ColumnDataSource"},"glyph":{"id":"30539","type":"Circle"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"30540","type":"Circle"},"selection_glyph":null,"view":{"id":"30542","type":"CDSView"}},"id":"30541","type":"GlyphRenderer"},{"attributes":{"source":{"id":"30462","type":"ColumnDataSource"}},"id":"30542","type":"CDSView"},{"attributes":{},"id":"30526","type":"ResetTool"},{"attributes":{"formatter":{"id":"30545","type":"BasicTickFormatter"},"ticker":{"id":"30479","type":"BasicTicker"}},"id":"30478","type":"LinearAxis"},{"attributes":{},"id":"30471","type":"LinearScale"},{"attributes":{"children":[[{"id":"30463","subtype":"Figure","type":"Plot"},0,0],[{"id":"30503","subtype":"Figure","type":"Plot"},0,1]]},"id":"30565","type":"GridBox"},{"attributes":{},"id":"30469","type":"LinearScale"},{"attributes":{},"id":"30519","type":"BasicTicker"},{"attributes":{},"id":"30545","type":"BasicTickFormatter"},{"attributes":{},"id":"30484","type":"WheelZoomTool"},{"attributes":{"ticker":{"id":"30474","type":"BasicTicker"}},"id":"30477","type":"Grid"},{"attributes":{"overlay":{"id":"30550","type":"BoxAnnotation"}},"id":"30485","type":"BoxZoomTool"},{"attributes":{},"id":"30547","type":"BasicTickFormatter"},{"attributes":{"formatter":{"id":"30558","type":"BasicTickFormatter"},"ticker":{"id":"30514","type":"BasicTicker"}},"id":"30513","type":"LinearAxis"},{"attributes":{},"id":"30486","type":"ResetTool"},{"attributes":{},"id":"30548","type":"UnionRenderers"},{"attributes":{},"id":"30514","type":"BasicTicker"},{"attributes":{"callback":null},"id":"30467","type":"DataRange1d"},{"attributes":{},"id":"30549","type":"Selection"},{"attributes":{"callback":null},"id":"30465","type":"DataRange1d"},{"attributes":{},"id":"30524","type":"WheelZoomTool"},{"attributes":{"ticker":{"id":"30514","type":"BasicTicker"}},"id":"30517","type":"Grid"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"30550","type":"BoxAnnotation"},{"attributes":{},"id":"30523","type":"PanTool"},{"attributes":{},"id":"30511","type":"LinearScale"},{"attributes":{"below":[{"id":"30473","type":"LinearAxis"}],"center":[{"id":"30477","type":"Grid"},{"id":"30482","type":"Grid"}],"left":[{"id":"30478","type":"LinearAxis"}],"plot_height":350,"plot_width":350,"renderers":[{"id":"30501","type":"GlyphRenderer"}],"title":null,"toolbar":{"id":"30490","type":"Toolbar"},"toolbar_location":null,"x_range":{"id":"30465","type":"DataRange1d"},"x_scale":{"id":"30469","type":"LinearScale"},"y_range":{"id":"30467","type":"DataRange1d"},"y_scale":{"id":"30471","type":"LinearScale"}},"id":"30463","subtype":"Figure","type":"Plot"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"30551","type":"BoxAnnotation"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#1f77b4"},"line_alpha":{"value":0.1},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y1"}},"id":"30540","type":"Circle"},{"attributes":{"callback":null},"id":"30505","type":"DataRange1d"},{"attributes":{"fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"xs_units":"screen","ys_units":"screen"},"id":"30552","type":"PolyAnnotation"},{"attributes":{"callback":null,"data":{"x":{"__ndarray__":"","dtype":"float64","shape":[300]},"y0":{"__ndarray__":"","dtype":"float64","shape":[300]},"y1":{"__ndarray__":"","dtype":"float64","shape":[300]}},"selected":{"id":"30549","type":"Selection"},"selection_policy":{"id":"30548","type":"UnionRenderers"}},"id":"30462","type":"ColumnDataSource"},{"attributes":{"fill_color":{"value":"#1f77b4"},"line_color":{"value":"#1f77b4"},"x":{"field":"x"},"y":{"field":"y1"}},"id":"30539","type":"Circle"},{"attributes":{"callback":null},"id":"30507","type":"DataRange1d"},{"attributes":{},"id":"30556","type":"BasicTickFormatter"},{"attributes":{"callback":null,"overlay":{"id":"30552","type":"PolyAnnotation"}},"id":"30489","type":"LassoSelectTool"},{"attributes":{},"id":"30509","type":"LinearScale"},{"attributes":{},"id":"30558","type":"BasicTickFormatter"},{"attributes":{},"id":"30487","type":"SaveTool"},{"attributes":{"overlay":{"id":"30561","type":"BoxAnnotation"}},"id":"30525","type":"BoxZoomTool"},{"attributes":{"dimension":1,"ticker":{"id":"30519","type":"BasicTicker"}},"id":"30522","type":"Grid"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"30561","type":"BoxAnnotation"},{"attributes":{"formatter":{"id":"30556","type":"BasicTickFormatter"},"ticker":{"id":"30519","type":"BasicTicker"}},"id":"30518","type":"LinearAxis"},{"attributes":{"bottom_units":"screen","fill_alpha":{"value":0.5},"fill_color":{"value":"lightgrey"},"left_units":"screen","level":"overlay","line_alpha":{"value":1.0},"line_color":{"value":"black"},"line_dash":[4,4],"line_width":{"value":2},"render_mode":"css","right_units":"screen","top_units":"screen"},"id":"30562","type":"BoxAnnotation"},{"attributes":{},"id":"30527","type":"SaveTool"},{"attributes":{"formatter":{"id":"30547","type":"BasicTickFormatter"},"ticker":{"id":"30474","type":"BasicTicker"}},"id":"30473","type":"LinearAxis"}],"root_ids":["30568"]},"title":"Bokeh Application","version":"1.1.0"}}';
                  var render_items = [{"docid":"a31ae450-3732-4a22-ae8c-3a09e09111a4","roots":{"30568":"b5dc6748-4caa-432b-a1e8-0ec2b82d13e8"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        embed_document(root);
                        clearInterval(timer);
                      }
                      attempts++;
                      if (attempts > 100) {
                        console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        clearInterval(timer);
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {} // ensure no trailing comma for IE
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();